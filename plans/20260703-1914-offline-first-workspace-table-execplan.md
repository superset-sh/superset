...
3. Automations denormalize `hostId`/`projectId` onto the automation row at create time; the cloud-side `verifyWorkspaceInOrg` lookup is skipped when the full tuple is supplied and its code path is deleted in R3 (old clients use it until then). Org membership, host access, and project-in-org checks all remain; a host rejects runs for workspace ids it doesn't have.
...
- [x] (2026-07-05) Milestone 3 (R2): `useHostWorkspaces` fan-out — per-host `workspace.list` queries (local direct, remote via relay; ports-hook template), merged and live-patched from each host's new `workspace:changed` events (added to the workspace-client event bus), last-seen lists persisted per host to IndexedDB (idb-keyval), Electric read-through fallback for hosts that served nothing. Shared once via `HostWorkspacesProvider` in the `_authenticated` layout; exposes `cache` ops (resolveHostUrl / optimistic upsert / remove / invalidate) for the write paths.
- [x] (2026-07-05) Milestone 4 (R2): all ~25 `useLiveQuery(collections.v2Workspaces)` consumers migrated to the hook (joins reassembled via useMemo maps; `$synced` semantics mapped to `source === "host"`). Writes unify through the owning host: create goes direct to `workspaces.create` with an optimistic cache upsert; rename/branch/taskId via host `workspace.update` (extended with `branch`); delete's UI commit point is the host row removal (Electric `waitForWorkspaceDeleted` deleted); automation pin changes send the denormalized fields. The Electric `v2Workspaces` collection is now read-only fallback (mutation handlers removed). Desktop typecheck clean; 2045 desktop tests pass.
- [x] (2026-07-05) R2 offline drill (CDP, real dev desktop): with the dev API killed, the sidebar rendered from host fan-out; a workspace created via the host appeared in the sidebar within ~3s (workspace:changed → fan-out cache, zero cloud); offline destroy removed it from the sidebar immediately (cloudDeleted:false, tombstone queued for the reconciler). Milestones 3-5 acceptance met; R2 is implementation-complete. Milestone 6 (R3 deletion) intentionally NOT implemented — gated on desktop adoption per Decision 8.
- [x] (2026-07-05) Milestone 5 (R2): MCP `workspaces_list` now fans out — cloud `host.list` for discovery, per-host `workspace.list` over the relay with the minted user JWT (`ctx.bearerToken`/`ctx.relayUrl` already existed), unreachable hosts reported in `unreachableHosts` instead of failing; MCP automations create/update auto-derive the denormalized pin (`resolveWorkspacePin`) when the model omits `targetHostId`/`v2ProjectId`. CLI `workspaces list` fans out via a new `lib/host-workspaces` helper (local manifest host + relay hosts, per-host failures print stderr warnings, cloud-discovery failure falls back to the local host only; project-name filter/display now joins cloud `v2Project.list` client-side); CLI automations create/update resolve `--workspace` across hosts and send `targetHostId`+`v2ProjectId`, deferring to the cloud legacy path with a warning when no reachable host knows the id. SDK: automations param docs updated to prescribe the full pin; `workspaces.list` intentionally stays cloud-backed until R3 (see Decision Log). Typecheck + `bun test` green in all three packages; manual MCP `workspaces_list` drill against a live host still to run.
...
- Decision: SDK `workspaces.list` stays cloud-backed (`v2Workspace.list`) through R2 instead of fanning out host-side.
  Rationale: the SDK does have relay plumbing (`hostQuery`/`hostMutation`), but `list()` returns `APIPromise<Workspace[]>` (single-request promise with `projectName` joined server-side and server-side filters); converting it to a multi-host aggregation breaks the public return type and filter semantics. Revisit at R3 when the cloud endpoint is deleted and a breaking SDK version is unavoidable anyway.
  Date/Author: 2026-07-05 / implementation (Milestone 5).
...
3. PostHog: SUPERSEDED — capture stays cloud-side until R3 (see Decision Log). Dual-write keeps every create/delete flowing through the cloud router, so events keep firing with no double-counting; the capture moves into host-service in R3 when the router is deleted. (The originally-planned client-id gating was dropped: `create.id` is a plain API input, not a trustworthy origin marker.)
