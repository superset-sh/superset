...
| Boundary choice | Strong at | Superset would still own | Best role |
|---|---|---|---|
| ACP | Agent ↔ editor interoperability, prompts, tools, permissions, capabilities, and agent-session lifecycle | Multi-device subscriptions, host-wide state, nested threads, workflow and subagent fidelity, cursor recovery, and non-chat domains | Host ↔ vendor harness |
| AI SDK UI | Streamed message parts, tool and approval state, typed UI state, chat hooks, and reusable components | Host multiplexing, persistence, multi-client arbitration, pagination and search, watcher events, and recovery | Canonical projection ↔ product UI |
| Superset-owned contract | Exact product entities, one host control plane, deterministic convergence, and additive product evolution | Schemas, reducer, client store, compatibility policy, and regression tests | Public host ↔ client boundary |

### Why the host contract is product-owned

ACP and AI SDK solve valuable but narrower problems than this contract.

ACP standardizes communication between an editor/client and an agent. AI SDK
UI standardizes how one chat consumes and renders a model response stream.
Superset must synchronize an entire host across multiple clients: session
directory and metadata, nested threads, permissions, client tools, workflows,
host status, workspace lifecycle, and future host-owned domains.

Using ACP or AI SDK as the public host wire would therefore not remove a
Superset protocol. Superset-specific semantics would move into `_meta`, custom
data parts, extension methods, or a second side channel. The result would have
two nested envelopes, two versioning surfaces, and more intermediate state,
while the product contract would still be ours in practice.

The product-owned boundary gives us:

- explicit `subscribe` / `unsubscribe` semantics for many streams over one
  host connection;
- multi-device arbitration, including idempotent intents and first-resolution
  wins;
- first-class `Thread`, workflow, subagent, and future side-chat semantics even
  when the active harness does not expose them;
- one cursor, replay, reset, and cold-load discipline for every host control
  domain;
- room for search, pagination, file-watcher invalidations, workspace updates,
  and other host capabilities without pretending they are chat messages;
- one canonical reducer and one client replica instead of an upstream chat
  state machine wrapped inside a second product state machine.

The transport split follows the data:

- tRPC carries snapshots, queries, and idempotent commands;
- `/sessions/sync` carries small ordered control events and replay;
- `/terminal/*` remains a separate throughput-oriented data plane for raw PTY
  bytes.

This is not a rejection of OSS. Upstream implementations are inputs, not the
source of product truth. We can vendor AI Elements, reuse AI SDK interaction
patterns, and align canonical shapes with ACP where the semantics match.
Upstream types terminate at adapters rather than leaking into the public
contract.

ACP remains useful at the vendor boundary today because the Claude and Codex
adapters reduce integration work and are already proven. Moving an adapter
in-process or driving a vendor SDK directly is a reversible optimization when
adapter lag, lost semantic fidelity, process complexity, or debugging cost
justifies it. SDK and adapter bumps run through the same real-model gym,
deterministic protocol suites, and Maestro regression story before shipping.
...
2. ~~React bindings and the mobile rewire~~ — done (2026-07-12). Shipped:
   `packages/host-service-react` (provider + zustand-backed hooks +
   `useSessionTimeline` with incremental fold), the framework-free
   transcript fold at `host-service-sync/timeline` (canonical events →
   message/tool-call/plan items, thought-splitting, subagent nesting,
   pending-permission cards), and `packages/host-client`'s `sessions`
   module (command plane + `syncApi`/`syncUrl` facades). Mobile's chat
   screens keep their shipped UI but ride `sessions.*` + `/sessions/sync`
   over the relay (verified live end-to-end against a dev host through the
   relay tunnel — the relay WS proxy doesn't negotiate subprotocols, so
   the mobile socket factory requests none). DELETED with the rewire: the
   `acpSessions.*` router, the per-session `/acp-sessions/:id/stream`
   route, `host-client/acp`, session-protocol's `client`/`react` subpaths,
   and their surface tests. `packages/session-protocol` itself shrinks to
   the ACP-bridge contract (acp/api/envelope/fold/state) still imported by
   the AcpSessionManager + canonical translator — it goes with item 3, not
   before.

   **Follow-up shipped (2026-07-13): settings catalogs, pickers, and the
   full mobile plane kill.** The session entity gained `settingOptions` —
   harness-reported catalogs (`{id, name, kind: model|mode|effort|other,
   currentValue, options[{value,name,description}]}`) composed live from
   the adapter's config options (grouped selects flattened, mode
   synthesized from `currentMode` when not mirrored, booleans skipped as a
   v1 reduction). Catalogs ride the composed session (snapshots +
   `session_upsert`), never the event log; the translator's settings
   change-key includes the catalogs so a catalog-only rebuild still moves
   the log and re-pushes the entity. The mobile composer's mode/model/
   effort chips are back (generic `OptionPicker` over `settingOptions`,
   writes through `sessions.update`), verified on-sim: chip → native sheet
   → live model switch reflected in the chip. The NewChatWidget's
   new-workspace flow moved off the legacy `workspaces.create` agents
   sugar onto `workspaces.create` (no agents) + `sessions.create` +
   `submitTurn` (image attachments ride the first turn as canonical image
   blocks), with the widget's model pick passed as adapter aliases (the
   shared "claude" preset: fable/opus/sonnet/haiku — `SUPERSET_CHAT_MODELS`
   Mastra ids are gone from mobile). And the old planes are now fully
   deleted from the phone: the Mastra chat thread screen
   (`chat/[sessionId]` UI + `useChatThread` + cloud `chat_sessions`
   Electric collection + the `HostChatClient` chat facade) is gone, the
   canonical thread now owns the `chat/[sessionId]` route (the `chat/acp/`
   path segment is dead), and the session-row menu's rename/archive go
   through `sessions.update`. Verified end-to-end on-sim: widget → new
   workspace → canonical thread → real agent reply, with the picked model
   active.

   **Follow-up shipped (2026-07-13): chat scroll discipline + thread
   polish.** The `Conversation` component now implements the ChatGPT-class
   scroll contract — the list moves only on user intent: drag-begin kills
   following instantly; re-pinning requires a real gesture resting at the
   true end (programmatic scrolls also emit momentum-end and must never
   re-pin); send anchors the message to the top with whitespace for the
   reply (bottom-relative target + corrective settle legs, exact even when
   sent from deep in history); streaming, content re-measure dips, the
   permission stack appearing/resolving, and the keyboard hiding never
   shift an unpinned reader — trailing scroll range is pre-banked as a
   native `contentInset.bottom` sized against the largest viewport seen
   plus dip slack (insets fire no layout events, which kills the
   spacer-view feedback loop), and every scroll target derives from the
   native content height, never LegendList's estimates
   (`initialScrollAtEnd`/`maintainScrollAtEnd`/`scrollToEnd` are banned —
   they chase estimated totals into blank space). Invariants are documented
   in `apps/mobile/components/ai-elements/AGENTS.md` and enforced by a
   hidden regression lab (`superset://debug/conversation-lab` +
   `.maestro/flows/conversation-lab.yml`) whose five scripted scenarios
   (pin-follow, no-move-unpinned with dips, overlay resolve, send anchor,
   send anchor from history) self-assert PASS/FAIL on-device. Thread
   polish riding along: tool rows title with the model-written command
   `description` (raw command stays in the detail sheet), GitHub-style
   +added/−removed line badges on file-edit tool rows (input-derived,
   tested), the running tool title breathes with a seam-free sine pulse,
   the working indicator is a caret-sized boids flock (six dots milling in
   an elliptical pen, speed-fade alpha, UI-thread `useFrameCallback` port
   of the web caret), and the glass header title widens to 70% of the
   screen before truncating. Cold thread load paints the FULL last page in
   one frame: `sessions.get` now resurrects on read so the snapshot always
   carries the recent tail + real head (the old zero-head fallback made the
   hub stream the entire replayed transcript event-by-event — the thread
   visibly built up from the first message), the thread holds a spinner
   until that one-shot page lands, and the list mounts invisible until the
   page is measured and parked at the bottom (quiet-gate + hard cap) —
   older history stays `getEvents` scrollback territory.
3. Direct Claude SDK adapter behind the same surface (prototype on the
   ...
   through the same real-model story, switch, then remove the ACP bridge
   (AcpSessionManager, translate-acp, and the rest of
