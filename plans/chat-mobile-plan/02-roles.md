...
Out-of-scope roles for this PRD (mentioned in desktop chat but not relevant to mobile-chat v2): **Admin** (no admin-level controls on mobile), **Dispatcher** (no Slack-style cross-system orchestration UI on mobile), **Subagent** (rendered read-only via UC-RENDER-06; not interactive).
