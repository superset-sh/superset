...
| Navigation | **NAV** | Shell-level navigation for the Chat tab: sessions list (default landing), workspace sections with collapse/expand, host-picker bottom sheet (Slack-style one-host-at-a-time), FAB → workspace-picker → new session, push-notification deep-link routing with host alignment, and empty-state handling for no-hosts / no-workspaces / no-sessions. Resolves the open Technical Sub-Decision #6 (host selection and workspace→host resolution). See `09-uc-nav.md` for canonical wireframes. |
