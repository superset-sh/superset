...
| 4 | Renderer read path | **Renderer fan-out hook + IndexedDB cache, no TanStack collection.** The renderer runs `workspace.list` against each known host in parallel (ports-hook template), merges, and live-updates from each host's new `workspace:changed` events; per-host query caches persist to IndexedDB so remote machines' last-seen lists survive restarts. The local host needs no cache — its daemon is reachable offline, so own-machine workspaces are always live. Consumers migrate off `useLiveQuery(collections.v2Workspaces)`. Nothing v1 (`packages/local-db`) and no host-side peer cache. *(Revised 2026-07-04 twice: local.db mirror → host.db peer cache → renderer fan-out + IndexedDB, per Decision 2's client-owned rationale.)* |
...
| 8 | Rollout | **Staged with read-through fallback; legacy cloud path lives until old clients are gone.** R1: host.db schema expands, host backfills full fields from cloud, dual-write. R2: desktop reads flip to renderer fan-out; rows no host returned fall back to still-synced cloud rows. R3: cloud surface (dual-write, router, shape, pages, table) removed only when telemetry shows old-client usage is negligible — the gate is adoption, not a date. |
