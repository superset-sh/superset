...
- `bun run test:cli-e2e` recreates the desktop-closed acceptance run from an isolated Git repository, Superset home, manifest, host database, attachment root, host-service process, and production-ABI PTY daemon. It writes scrubbed command results, assertions, service logs, and the raw fake-agent byte capture under ignored `test-results/cli-agent-sessions/`.
