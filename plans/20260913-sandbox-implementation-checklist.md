...
- [x] Boot timing off the row and repositories unordered (migrations 0119, 0120; decisions 27, 28). On the dev stack after the change: a single-repository create came up ready with `acme-demo@.` marked primary, and the provision log line recorded `claim 702 ms, create 10505 ms, settle 1329 ms, total 12747 ms`. The Sentry transaction cannot be seen in dev (the API's Sentry is enabled in production only); confirming it arrives is owed after the production deploy, and the log line goes once it does.
