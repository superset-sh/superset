...
> **Status: selected approach.** This is approach **B** of two competing designs for replacing the mastra harness, chosen on 2026-07-09. Approach **A** — [a direct Claude Agent SDK runtime with a custom envelope protocol](./session-harness-claude-agent-sdk.md) — is preserved as the fallback design; its delivery-guarantee mechanism (seq/cursor journal) and unified-permission model carried over into this plan.
...
- **Q3 — Unifying workspace "artifacts" (deliberately deferred).** A workspace now accumulates several independently-built openable things: terminal sessions, mastra chat sessions, ACP sessions, and soon likely browser windows and other named surfaces. Each has its own list, store, and screen — exactly how chat and terminal tabs were treated before, and that disjointedness is acceptable for now. The eventual shape is a discriminated union (`{ kind: "terminal" | "chat" | "acp-session" | "browser", id, title, ... }`) backing one "artifacts/tabs in this workspace" list, with per-kind renderers so switching between kinds stays obvious at the rendering level. No such abstraction exists in the repo today (desktop tabs come closest but are desktop-local state); building it is out of scope for this plan — revisit when a third-or-fourth artifact kind lands on mobile.
