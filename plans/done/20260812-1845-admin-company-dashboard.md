...
D-12 amendment (2026-09-24): `admin.deleteUser` was kept, not purged. Support uses it for deletion requests made by email, and it now shares `purgeAccount` (`packages/trpc/src/lib/account-purge/`) with the daily `purge-expired-deletions` job in `apps/api`. Do not remove it under the D-11 default.
