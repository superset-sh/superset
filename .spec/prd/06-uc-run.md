...
- ☐ (No-overwrite-user-edits — post-PRD analysis) The rename branch (every Nth user message after the first) MUST NOT overwrite a title that the user has explicitly set via the rename UI. The session record carries an `isAutoGenerated` / `renamedByUser` flag (or the host-service compares the current title against the most recent auto-generated value it remembers); if the current title diverges from the last auto-generated value, the auto-rename is skipped and a `console.info` note records the skip for triage.
