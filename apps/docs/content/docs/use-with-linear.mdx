...
## Linking the workspace back into the issue

Linear only accepts HTTP(S) links, so a `superset://` deep link can't be pasted
into an issue or a comment. `workspaces open` returns a public HTTPS handoff
URL as `webUrl` for exactly this:

```bash
link="$(superset workspaces open "$id" --print --json | jq -r '.webUrl')"
# https://app.superset.sh/open/v2-workspace/<workspace-id>
```

Post that link on the issue. Opening it attempts the native handoff to the
reader's own desktop app, and leaves an **Open Superset** retry plus a download
link when the app isn't available. The page requires no sign-in and looks
nothing up — it only hands off the workspace ID, so the desktop app is still
what resolves the workspace and what shows a missing or inaccessible one. That
also makes the link safe to use from an email or a GitHub comment. Use `webUrl`
rather than assembling the URL yourself.
