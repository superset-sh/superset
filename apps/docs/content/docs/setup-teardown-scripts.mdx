...
If no commands are configured for a key, Superset looks for a script instead: `.superset/<key>.sh` (`setup.sh`, `teardown.sh`, `run.sh`) in the worktree first, then the main repo. Gitignored scripts only exist in the main repo, since worktrees skip gitignored files.
