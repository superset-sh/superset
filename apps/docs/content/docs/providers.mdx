...
[OrcaRouter](https://www.orcarouter.ai) is an OpenAI- and Anthropic-compatible gateway that fronts
models from many vendors behind one API key. It also runs gateway-level, zero-trust security for AI
agents on the same endpoint — screening every prompt and response and governing every tool call on a
default-deny basis, with no changes to your agent.
...
<Callout type="warn">
OrcaRouter namespaces model IDs by vendor, so the default Claude model names don't resolve. Pin the
namespaced IDs:
