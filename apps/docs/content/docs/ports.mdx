...
- Ports can be opened at `localhost:PORT` from the browser action ([local workspaces only](#remote-access))
...
When a workspace lives on a [remote host](/remote-access), the sidebar lists ports listening **on that host**. Superset doesn't forward them to your machine yet (it's on the roadmap), so `localhost:PORT` only works on the host itself.
