...
**Fix:** add [setup scripts](/setup-teardown-scripts) to `.superset/config.json`:
...
**Fix:** on the host, confirm the toggle in **Settings → Security** is on (desktop) or that `superset start --daemon` is running (CLI). See [Remote Workspaces](/remote-workspaces).
...
**Fix:** none needed for one-off skips. If it keeps happening, retarget the automation to a device that stays online. See [Automations](/automations).
