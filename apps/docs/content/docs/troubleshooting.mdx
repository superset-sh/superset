...
## App Starts Without a Window

If the startup error names a corrupt `tanstack-db.sqlite`, the app can fail before its automatic updater starts. The fix removes this unused database from startup. Install a release containing the fix manually from [superset.sh/download](https://superset.sh/download); if no newer release is available, [contact support](/community) with the startup error.
...
**Fix:** on the host, confirm the toggle in **Settings → Remote Access** is on (desktop) or that `superset start --daemon` is running (CLI). See [Remote Access](/remote-access).
