...
On the automation's detail page, schedules live in a **Triggers** list. Each trigger is a row reading like a sentence (e.g. *Daily at 09:00 PDT*), with the next run shown inline. **Add Trigger** adds another schedule: one automation can fire on several cadences, and every automation needs at least one trigger. Hover a row for the remove button. Edits are drafts until you press **Save triggers** (or **Discard**).
