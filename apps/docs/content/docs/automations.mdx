...
The automations list shows a **Last run** column (ran / running / failed) so you can spot problems without opening each automation, and the sidebar Automations tab badges a count of automations whose last run failed.

If **Run now** or a scheduled run is skipped because the target device isn't connected to the relay, Superset shows a dialog explaining why and, if the target is this device, an inline **Enable relay access…** shortcut to Settings → Security.
...
- **Offline hosts skip**: If your device is offline at fire time, the run is marked `skipped_offline` and the next occurrence is scheduled normally. The automation's Last run status and run-history tooltip surface this so it isn't silent.
