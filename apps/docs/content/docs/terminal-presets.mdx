...
## Agent-Linked Presets

Link a preset directly to an agent definition so its commands stay in sync with your agent settings.

In **Settings → Terminal**, open any preset and click **Import agent** to choose from your configured agents. The preset's command fields are populated from **Settings → Agents** — edits there propagate automatically to all linked presets.

Built-in terminal agents are auto-seeded on first use.
