...
The bar also ships a built-in **Superset CLI** preset that opens a terminal running `superset --help` — a starting point for scripting workspaces, agents, and automations with the [CLI](/cli/getting-started). Built-in presets can't be edited or reordered, but you can hide them and restore them later.
