...
To search and read documentation, connect to the public documentation server at `https://docs.superset.sh/mcp`. See [Ask an agent](/ask-an-agent) for its search tools and Markdown endpoints.
...
| Tool | Description |
|------|-------------|
| `pages_list` | Search and paginate pages, optionally scoped to one workspace |
...
| `pages_publish` | Publish an HTML document (passed inline, not as a path) and get its shareable URL |
...
`pages_list` returns `{ items, nextCursor }`, ordered newest-created first.
Pass `search` to match title, slug, or description, `limit` for 1–200 rows
(default 50), and the returned `nextCursor` as `cursor` to continue. Keep
fetching until `nextCursor` is null. `scope` accepts `all` (default), `team`
for shared pages, or `mine` for your private pages.

`pages_publish` accepts `visibility: "just_me"`, `"org"` (the default for new
pages), or `"everyone"` for anonymous read-only access. Pass `workspaceId` and
`entryPath` together to link the page to a workspace. Both are optional;
outside a workspace, save the returned `id` and pass it as `pageId` when
publishing updates. Otherwise each publish creates a separate page.
