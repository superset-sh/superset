...
When Superset detects a port in use (e.g. `localhost:3000`), clicking its external link icon opens it based on your **Settings → Links → Ports** preference (in-app browser, new in-app tab, or your default browser). See [Port Management](/ports) for details.
