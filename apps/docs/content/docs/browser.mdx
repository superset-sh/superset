...
Design mode turns UI feedback into agent context. Click any element in the page and describe the change; the agent receives the element's markup, styles, and a screenshot instead of a prose description.

![Design mode capturing a button and describing the change](/images/design-mode.png)

1. Click **Design** in the browser toolbar. Hovering now highlights the element under the cursor.
2. Click an element to capture it. A prompt card opens next to it.
3. Describe the change, pick an agent (an active session or a new one), and press **Enter**.

The agent receives a one-line prompt plus a context file staged in `.superset/attachments/`:

- The element's selector, HTML, computed styles, and accessibility info
- React component names, props, and the source file, when the page is a dev build
- A cropped screenshot of the element
- CLI commands to reload, inspect, and screenshot the live pane, so the agent can verify its own change

Press **esc** or click anywhere on the page to discard a capture and pick a different element. Leave design mode with the **Design** button or the close button in the banner.
