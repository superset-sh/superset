...
In the prompt-first flow, describe the task, pick an agent, and create. With **Auto-run command** on, the agent starts working immediately.
...
Future workspaces will then install dependencies and copy env files automatically via [setup scripts](/setup-teardown-scripts).
...
Delete the other workspaces. [Teardown scripts](/setup-teardown-scripts) run automatically, and the worktrees are removed. Deleting a workspace after merging is the normal rhythm, not an edge case.
