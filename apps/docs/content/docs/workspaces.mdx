...
The workspaces page toggles between **List** and **Board** layouts; Board is the default. Board columns follow workspace state, derived from agent status and PR state: **Idle**, **Working**, **Needs attention**, **Needs review**, **Merged**, and **Deleted**. Merged and Deleted are history: deleted workspaces keep a card instead of vanishing. Both lanes hide themselves while they're empty and the Archived window is **Hidden** (the default).

The toolbar collapses everything into two menus, shared by both layouts:

- **Filter** - Project, PR state, Agent, Pinned, and Device. **Pinned** narrows to **All workspaces**, **Pinned**, or **Unpinned**. Active filters show a count badge and sync to the URL.
- **Display** - **Sort by** (Last activity, Created, Diff size, Name), the **Archived** window (Hidden, Past week, Past month, All), and, in Board view, **Lanes**: a checkbox per column to hide or show it. Hidden lanes come back by re-checking them; lane visibility is board-only, stays out of the URL, and resets on restart.
