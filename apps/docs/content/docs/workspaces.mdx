...
Hover a workspace and click the branch name to rename it locally via the hover card. The GitHub link remains available as a separate icon next to the branch name.

## Mark as Unread

Right-click a workspace → **Mark as Unread** to flag it with a green attention dot. The dot clears automatically when you open the workspace.
