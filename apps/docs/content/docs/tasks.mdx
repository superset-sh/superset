...
- **Table view** with filtering and sorting by status, priority, and assignee, plus explicit task-state filters (All tasks, Active, Backlog, Todo, In progress, Done, and Canceled)
- **Repository filter**: a multi-select picker in both views; leaving it empty means all repositories
...
The **Pull requests** view lists PRs from your projects' repositories, with its own repository (multi-select), state, author, review-status, and search filters — review status includes review requests for you personally and for your teams. PR rows show a compact CI check summary, and the detail view includes a full **Checks** section, so you don't have to leave Superset to see whether CI is green. PR-based workspaces also show review status and CI checks in the sidebar. See [Workspaces](/workspaces#pr-status) and the [Diff Viewer](/diff-viewer) for the review loop.
