...
Each file header in the diff pane has actions on hover:

- **Expand/collapse** — show or hide the file's diff
- **Mark as viewed** — flag a file as reviewed
- **Copy path** — copy the file path to clipboard
- **Discard** — discard unstaged changes for that file

## Expand Unchanged Regions

Toggle **Expand unchanged regions** in the diff pane header to show or hide unmodified lines around each change. The preference is persisted across sessions.

## Jump to Comment

Click a review comment in the sidebar to scroll the diff to that line. The diff anchors to the correct side — deletion-side comments land on the deletion column, addition-side comments on the addition column.
