...
Press **⌘F** (Ctrl+Shift+F) to search across the whole changeset, not just what's currently on screen.

- Matches are counted across every file, including ones scrolled out of view
- Enter/Shift+Enter cycle to the next/previous match, jumping across files as needed
- Landing on a match in a collapsed file expands it automatically
