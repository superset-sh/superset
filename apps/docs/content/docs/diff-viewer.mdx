...
## Changes Sidebar View Modes

The changes sidebar has two grouping modes, toggled with the icons in the sidebar header:

- **Folders** (default) — files grouped one level deep by parent folder; root-level files appear under a "Root Path" header
- **Tree** — full directory hierarchy with expand/collapse, git-status colors, and file-type icons

Both modes support clicking a file to open its diff, right-clicking for context menu actions, and `⌘`-click to open in a new tab. The selected mode persists per workspace.
