...
Install [Superset for iPhone](https://apps.apple.com/app/id6788926383). Mobile requires **iOS 26 or later** and **Superset Pro**.

1. On your computer, enable **Allow remote access to this device via relay** in **Settings → Remote Access**. You can also use the inline toggle in **Settings → Mobile** when that tab is available. Both toggles control the same setting.
2. On your phone, sign in with the same Superset account and select the same organization. The QR code in desktop Mobile settings opens the App Store; it does not sign you in.
3. Select your computer and open a workspace to continue its terminal sessions, send prompts, review diffs, or merge PRs. Keep the computer awake, online, and running Superset.

A computer that has never connected shows **One step left** with Remote Access instructions. A previously connected computer shows its last-seen time when offline. **Can’t check** means presence could not be loaded; use **Try again**. See [Remote Host Won’t Connect](/troubleshooting#remote-host-wont-connect).
