...
When launching an agent task, a toggle controls whether the command runs immediately or is staged for review:

- **On** - Agent command runs automatically
- **Off** - Terminal agents stage the command without pressing Enter; chat agents pre-fill the input for manual send
