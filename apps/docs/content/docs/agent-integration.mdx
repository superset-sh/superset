...
If an agent's terminal dies under it (a crash, a reboot, a killed process), the pane shows an "*Agent* was interrupted" banner with a **Resume** button. Resume relaunches the agent with its resume command (e.g. `claude --resume <session-id>`) in the same pane, restoring the conversation.

![The resume banner on an interrupted agent terminal](/images/agent-resume-banner.png)
