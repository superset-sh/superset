...
If an agent's terminal dies without the agent exiting cleanly — a crash, a reboot, a killed process — the pane shows an "*Agent* was interrupted" banner with a **Resume** button. Resuming relaunches the agent with its own resume command (e.g. `claude --resume <session-id>`) in the same pane, restoring the full conversation.

This works for agents with an id-based resume command (configurable via **Resume Args** under **Settings → Agents**), including agent sessions carried over from v1 pane migration. From the CLI:
