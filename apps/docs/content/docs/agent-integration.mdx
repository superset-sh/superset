...
When a workspace has more than one running agent, the sidebar shows a chip per agent. Right-click a chip for:
- **Open Agent** - Jump to that agent's terminal
- **Kill Agent** - Stop the agent's session
