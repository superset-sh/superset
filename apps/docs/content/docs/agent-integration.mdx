...
- **Mistral Vibe** - Mistral's coding agent
...
Terminal agent panes, tabs, and sidebar workspace rows show a status dot:

- **Amber pulsing** - the agent is working
- **Amber "permission"** - waiting on your approval
- **Red pulsing "failed"** - the agent's CLI reported an API-level failure (rate limit, overloaded, auth, billing, server error). Currently reported by Claude Code; clears automatically the next time you run the agent.

Failed and permission states count toward unread and dock-badge attention totals.
