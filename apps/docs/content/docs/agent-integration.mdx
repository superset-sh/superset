...
When accessing a workspace through the web app, an agent bar appears above the terminal area. It shows your configured agents as one-click launchers — click an agent to open a terminal session running that agent immediately.

Agent availability in the bar reflects what's configured in **Settings → Agents** on the host machine serving the workspace.
