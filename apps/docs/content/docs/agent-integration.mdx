...
When an agent is running in a terminal pane, the pane tab shows that agent's logo instead of the generic terminal icon. The icon reverts when the agent session ends.
