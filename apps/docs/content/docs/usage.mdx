...
Superset reads quota from the providers' usage endpoints using the agent credentials stored on your host. Some plans (org-managed, education) don't expose limits.
...
- **Add account** creates a second profile. Pick a profile name and billing type, then complete Claude Code or Codex sign-in in the terminal inside the dialog. Click a login URL to open your system browser. Superset detects a successful sign-in automatically. For a remote host, copy the shown command and run it in a terminal on that host.
...
- **Per project**: a project can pin its own account under Project settings → **Agent accounts** (say, a work login for one repo and a personal one for another). It overrides the host default for agents and terminals in that project's workspaces, and reaches already-open terminals the same way a host-wide switch does. Sessions always use the host default.
...
Joining or rejoining the leaderboard publishes available local usage history
from the last 90 days, starting no earlier than July 29, 2026. Leaving deletes
your published history. Rejoining restores only the history within that window
that is still available in your local session logs.
