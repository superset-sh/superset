...
The command returns a shareable URL. On the first publish from a workspace, it also attempts to open the page according to **Settings → Links → Pages**. Republishing does not reopen it. Automatic opening is skipped over SSH, in cloud sandboxes, and when publishing without a workspace.
