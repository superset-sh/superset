...
The hooks are scoped to Superset terminals: they check for a Superset-supplied environment variable and no-op instantly in sessions launched anywhere else. To remove Superset's entries from an agent's global config entirely, turn off that agent's **Superset hooks** toggle in **Settings → Agents**.
