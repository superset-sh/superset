...
## Connected Service Plugins

The plugin catalog includes Notion, Slack, and Gmail. Install a plugin from
**Plugins**, then connect the account it needs. Connections are separate from
installation: installed skills alone do not authorize access to a service.
If a connection expires, reconnect it when prompted.

Agents call connected tools through Superset's credential proxy. The service
credential stays on the server; it is not written into local agent config.
You can inspect and call the same tools with [`superset mcp`](/cli/cli-reference#mcp).
