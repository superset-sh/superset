...
The ports sidebar shows ports listening **on the remote host**. When you select a remote workspace, the desktop app forwards each of its ports to the same port number on your machine, so `localhost:3000` reaches the dev server on the host. Hot reload over WebSocket works. When you select another workspace, the app stops those forwards and starts the new workspace's forwards.
...
Forwarding needs a host on the current relay (host-service 1.20.2 or newer). For a host you reach some other way, [forward a port yourself](/ports#forward-a-port-yourself) with SSH or Tailscale.
