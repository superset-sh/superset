...
<Command
  ...
  hostId: string;
	...
	humanColumns={["NAME", "BRANCH", "PROJECT", "GROUP", "ID"]}
	quiet="workspace IDs"
>
...
project id when the host doesn't know the project's name. `GROUP` is the
...
>
Show details for a single workspace on its host. `groupId`/`groupName` are
`null` when the workspace isn't in a sidebar group. `--field` is handy in
...
<Command
	...
	options={[
		...
		{ flag: "--host <id>", description: "Host the workspace lives on. Defaults to this machine." },
		{ flag: "--local", description: "Target this machine (the default)." },
	]}
	output={`{ id: string }`}
>
Reorder a workspace within its sidebar list — its group if it's grouped,
otherwise the project's top-level order. Specify exactly one position.
...
across your devices. Every group command targets one host — this machine by
default, or `--host <id>` for another. Deleting a group never deletes
workspaces — they just become ungrouped.

<Command
	name="superset workspaces groups list"
	options={[
		{ flag: "--host <id>", description: "Target host machineId. Defaults to this machine." },
		{ flag: "--local", description: "Target this machine (the default)." },
