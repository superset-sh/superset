...
<Command
	...
	options={[
		...
		{ flag: "--attachment-id <uuid>", description: "Pre-uploaded host attachment UUID for the spawned agent. Repeatable." },
	]}
	output={`{
  workspace: Workspace;
...
`--agent`/`--prompt`. Pass either or both. Attachments require `--agent`:
local paths are uploaded to the target host, while `--attachment-id` reuses a
file already uploaded there.
