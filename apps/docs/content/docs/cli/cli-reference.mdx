...
The desktop app must be open: every mutation waits for the renderer to apply it
and acknowledge the resulting state. Exact project, workspace, and group names
can be used in place of IDs; ambiguous names fail with the matching IDs.
...
ungrouped project area. Sidebar state belongs to the local desktop app, so these
commands do not accept remote host flags and do not run while the renderer is
closed.
