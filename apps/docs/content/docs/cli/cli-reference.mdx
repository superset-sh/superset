...
<Command name="superset environments list">
List the active organization's cloud environments, including their IDs, names,
and repositories. Human-readable output marks unusable environments. JSON
returns the environment array, including entries that cannot start a workspace.

```bash
superset environments list --json
```
</Command>
