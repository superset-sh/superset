...
Set up and discover hosts registered to the active organization. To control the
host server running on this machine, use [`start`](#start),
[`stop`](#stop), and [`status`](#status).

<Command
	name="superset hosts add <target>"
	args={[
		{ name: "target", required: true, description: "SSH destination: `[user@]host[:port]`, including bracketed IPv6 (`user@[::1]:22`)." },
	]}
	options={[
		{ flag: "--org <id>", description: "Organization to register the host under (id, slug, or name). Required when you belong to more than one and there is no TTY to prompt on." },
		{ flag: "--identity <path>", description: "SSH private key to authenticate with (`ssh -i`)." },
		{ flag: "--port <number>", description: "SSH port. Overrides a `:port` in the target." },
		{ flag: "--version <version>", description: "CLI version to install, e.g. `1.30.2`. Default: the latest release." },
		{ flag: "--reinstall", description: "Reinstall even if Superset is already present on the machine." },
		{ flag: "--allow-shared-machine-id", description: "Proceed on a machine whose `/etc/machine-id` is empty, accepting host id collisions." },
		{ flag: "--timeout <seconds>", description: "How long to wait for the host to come online. Default: 90." },
		{ flag: "--yes", description: "Skip the confirmation prompt. Required when there is no TTY." },
	]}
>
Set up another machine over SSH and connect it as a host. Installs the CLI
there if it is missing, stores a freshly minted API key scoped to that device,
starts the host server, and waits until the cloud reports the host online.

Requires SSH access to the target and `curl` on it. Linux and macOS, x64 and
arm64 — the platforms [the installer](/docs/cli/getting-started) publishes.
Nothing has to be done at the far end, and nothing needs the desktop app: a
host claims itself the moment its relay tunnel opens.

```bash
superset hosts add ubuntu@devbox --org acme --yes
superset hosts add ubuntu@devbox:2222 --identity ~/.ssh/id_devbox
```

The machine stays registered across reboots, but the host server itself does
not come back on its own — `--daemon` survives the SSH session closing, not a
restart. Run it under a supervisor (systemd with `Restart=on-failure`, or
launchd) for a box that must always be reachable.

<Callout type="warn">
	A Linux machine with an empty `/etc/machine-id` — common in containers with
	no systemd, and in VMs cloned from a template — derives the same host id as
	every other such machine, and they evict each other's relay tunnel in a
	loop. This command refuses those by default; give the machine its own id
	with `systemd-machine-id-setup` instead of forcing past it.
</Callout>

<CommandReturns>
The new host's id, and whether the cloud has seen it come online yet.

<HumanOutput>
```text
┌  superset hosts add (Acme)
│
◇  devbox: Linux x86_64, Superset not installed
│
◇  Host service running on devbox
│
◇  acme-devbox is online
│
└  acme-devbox is ready — it shows up in the app and in hosts list.
```
</HumanOutput>
...
{
  hostId: string;
  hostName?: string;       // present once the host is online
  organizationId: string;
  online: boolean;         // false if the wait timed out
  installed?: boolean;     // whether this run installed the CLI
}
```
</JsonOutput>
</CommandReturns>
...
<Command
  ...
  online: "yes" | "local" | "no";
	...
	quiet="host IDs"
>
List hosts registered to the active organization. A machine registers itself
when `superset start` runs on it, or when
[`hosts add`](#superset-hosts-add) sets it up for you over SSH.
...
<Command
	name="superset agents read"
	options={[
		{ flag: "--workspace <id>", required: true, description: "Workspace ID." },
		{ flag: "--terminal <id>", required: true, description: "Terminal ID, returned as sessionId by agents create." },
		{ flag: "--host <id>", description: "Target host. Mutually exclusive with --local." },
		{ flag: "--local", description: "Read from this machine." },
	]}
>
Read the agent's saved transcript, beyond the visible screen returned by
`terminals read`. Without `--host` or `--local`, the target defaults to cloud
when your account has cloud workspaces, otherwise this machine. The command
fails if Superset cannot read a transcript for that terminal.

```bash
superset agents read --local --workspace <workspace-id> --terminal <terminal-id>
```

<CommandReturns>
The transcript object in JSON mode, or its text in human-readable mode.
</CommandReturns>
</Command>
...
`claude-opus-5-5`. Codex includes `gpt-6-sol` and `gpt-6-luna`. The host rejects an unknown id before launching the agent,
...
| Codex | `low`, `medium`, `high`, `xhigh`; `max` and `ultra` depend on the selected model |
| Mastracode | `off`, `low`, `medium`, `high`, `xhigh` |
| Pi | `off`, `minimal`, `low`, `medium`, `high`, `xhigh` |
| Copilot | `low`, `medium`, `high`, `xhigh` |

GPT-6 Sol supports `max` and `ultra`; GPT-6 Luna supports `max`.
...
>
Evaluate JavaScript in the page and return the result for
...
Publish and manage [Pages](/pages) in the active organization.

<Command
	name="superset pages list"
	options={[
		{ flag: "--workspace <name|id>", description: "Scope to a workspace. Defaults to SUPERSET_WORKSPACE_ID; pass an empty string for the whole organization." },
		{ flag: "--search <text>, -q <text>", description: "Search titles, slugs, and descriptions." },
		{ flag: "--limit <n>", description: "Fetch one batch of 1–200 pages." },
		{ flag: "--cursor <cursor>", description: "Continue from the previous nextCursor." },
	]}
>
Without `--limit` or `--cursor`, fetches all matching pages and returns a plain
array. Either pagination flag selects one batch and returns
`{ items, nextCursor }`; `nextCursor` is null at the end. A cursor without a
limit uses 50 rows per batch. Pages are ordered newest-created first.
`--quiet` prints page IDs in either mode. Linked workspaces appear in the
WORKSPACE column and in each row's `workspaceLinks`.

```bash
superset pages list --workspace '' --search onboarding --json
superset pages list --workspace '' --limit 20 --json
superset pages list --workspace '' --limit 20 --cursor '<nextCursor>' --json
```
</Command>

<Command
	name="superset pages publish <path>"
	args={[
		{ name: "path", required: true, description: "HTML file, or directory containing index.html and its assets." },
	]}
	options={[
		{ flag: "--title <text>", description: "Page title; defaults to the file or directory name." },
		{ flag: "--description <text>", description: "Short description." },
		{ flag: "--label <text>, -l <text>", description: "What changed in this version." },
		{ flag: "--visibility <value>", description: "just_me, org, or everyone. New pages default to org." },
		{ flag: "--page <id>", description: "Add a version to this existing page." },
		{ flag: "--workspace <name|id>", description: "Workspace link; defaults to SUPERSET_WORKSPACE_ID. Optional." },
		{ flag: "--no-watch", description: "Do not register this terminal to receive new page comments." },
	]}
>
Publishing the same entry path from the same workspace adds a version to the
existing page. Without a workspace, keep the returned ID and pass `--page` on
later publishes to avoid creating another page. Existing visibility is
preserved unless explicitly supplied.

The first version published from a workspace attempts to open in the desktop,
following **Settings → Links → Pages**. This is skipped over SSH or in a cloud
sandbox. An opening failure does not fail the publish; the result includes an
`openNote`. Later versions do not reopen.

```bash
superset pages publish report.html --title "Weekly report" --visibility everyone
superset pages publish report.html --page <page-id> --label "Addressed feedback"
```

<CommandReturns>
Page metadata including `id`, `title`, `version`, and `url`, plus `unanchored`,
`watching`, and asset upload/reuse counts. Unlinked publishes include a
`republish` command; watch and opening notes appear when applicable.
</CommandReturns>
</Command>

<Command
	name="superset pages delete <page>"
	args={[
		{ name: "page", required: true, description: "Page ID or slug." },
	]}
	options={[
		{ flag: "--yes", description: "Skip the interactive confirmation." },
	]}
	output="{ id: string }"
>
Delete a page and every version. Interactive terminals ask for confirmation
unless `--yes` is passed; non-interactive input does not prompt.
</Command>

### mcp

Use tools from connected plugins. This is separate from the
[Superset MCP server](/mcp-server), which exposes Superset itself to agents.

<Command
	name="superset mcp tools"
	options={[
		{ flag: "--plugin <name>", required: true, description: "Plugin name from superset plugins list." },
	]}
	output="Array<{ plugin: string; tool: string; description: string }>"
>
List the tools a connected plugin exposes. Use the returned names and
descriptions before calling a tool.
</Command>

<Command
	name="superset mcp call-tool <tool> [arguments]"
	args={[
		{ name: "tool", required: true, description: "Tool name from mcp tools." },
		{ name: "arguments", description: "JSON arguments; defaults to {}. Pass - to read stdin." },
	]}
	options={[
		{ flag: "--plugin <name>", required: true, description: "Connected plugin name." },
	]}
>
Call a plugin tool and return its MCP result, including the content array.
Credentials are attached by Superset's API. `--connection` and `--plugin-id`
are deprecated aliases for `--plugin`; they take a plugin name, not a connection ID.

```bash
superset mcp tools --plugin notion
superset mcp call-tool notion-fetch --plugin notion '{"id":"<page-id>"}'
```
</Command>
