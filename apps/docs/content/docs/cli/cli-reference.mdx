...
Organize user-created workspace groups in the desktop sidebar on this machine.
State is stored for the current user and organization on this device, so the
desktop app may be closed. Exact project, workspace, and group names can be used
in place of IDs; ambiguous names fail with the matching IDs. Name resolution
still requires the local host service (`superset start --daemon`).

<Command name="superset sidebar list">
List groups, their expanded or collapsed state, and workspace placement.
</Command>

<Command
	name="superset sidebar groups create <name>"
	args={[
		{ name: "name", required: true, description: "Group name." },
	]}
	options={[
		{ flag: "--project <name-or-id>", required: true, description: "Local project name or ID." },
	]}
>
Create a group at the end of the project's sidebar items.
</Command>

<Command name="superset sidebar move <workspace>" options={[
	{ flag: "--group <name-or-id>", description: "Move into this group." },
	{ flag: "--ungrouped", description: "Move back to the project root." },
]}>
Move a workspace by exact name or ID. Pass exactly one destination option.
</Command>

```bash
superset sidebar groups create "In review" --project superset
superset sidebar move "fix-login" --group "In review"
...
superset sidebar move "fix-login" --ungrouped
superset sidebar groups delete "Ready to merge"
```

Deleting a group is non-destructive: its workspaces remain in the sidebar's
ungrouped project area. Sidebar layout belongs to the local client profile, not
to any resource host, so these commands do not accept remote host flags. The
desktop renderer observes the same state when it is open or starts later.
...
`--agent`/`--prompt`. Pass either or both. Attachments require `--agent` and
local paths are uploaded to the target host before launch.

Workspace creation and the inline agent launch are separate outcomes. If the
workspace is created but the agent fails to start, the command exits non-zero,
prints the retained workspace ID, and suggests an `agents create` retry instead
of deleting the workspace or reporting full success.
...
superset workspaces create \
  ...
  --branch review/with-context \
  --local \
  --agent codex \
  --prompt "Review the attached trace" \
...
the workspace view until you navigate to it. Use
[`agents sessions list`](#superset-agents-sessions-list) to recover a live
terminal session ID. `workspaces open` can't focus a session, so build the deep
link yourself and append a query param chosen by the session `kind` returned by
`agents create`:
...
fresh terminal session on that host. A successful response means the configured
agent process started; immediate launch failures return an error instead of an
idle terminal session ID. Large, multiline, Unicode, and attachment-expanded
prompts bypass the interactive shell's line editor so their complete bytes reach
argv- and stdin-based agents.

Local `--attachment` paths are uploaded to the workspace's host before launch.
Use `--attachment-id` for files already uploaded to that same host. Both options
are repeatable and may be combined.
...
The session commands use these JSON shapes:
...
type AgentSession = {
  ...
  status: "working" | "permission" | "idle" | "failed";
};
...
<Command
	name="superset agents sessions list"
	options={[
		{ flag: "--host <id>", description: "Filter to one host machineId." },
		{ flag: "--local", description: "Filter to this machine." },
		{ flag: "--workspace <id>", description: "Filter to one workspace." },
	]}
	output={`Array<{
  status: "working" | "permission" | "idle" | "failed";
  agent: string;
...
>
List live terminal-agent sessions across reachable hosts. With no host filter,
the command queries online organization hosts plus a running local host service;
an unreachable host becomes a warning without hiding results from other hosts.
These commands control terminal agents only—Superset chat sessions are not part
of this live PTY interface.

```bash
superset agents sessions list
superset agents sessions list --local --workspace ws_…
```
</Command>

<Command
	name="superset agents sessions read <session-id>"
	args={[
		{ name: "session-id", required: true, description: "Full terminal session id." },
	]}
	options={[
		{ flag: "--lines <n>", description: "Logical lines to return (default 120, maximum 1000)." },
		{ flag: "--host <id>", description: "Probe only one host." },
		{ flag: "--local", description: "Probe only this machine." },
	]}
	output={`{
  terminalId: string;
  workspaceId: string;
...
>
Read a recent plain-text terminal snapshot without attaching to or focusing the
session. The snapshot is bounded and reports truncation in JSON mode; it is not
a canonical transcript. The terminal ID must be the full ID returned by
`agents create` or `agents sessions list`. Reads also work after a host-service
restart while the PTY daemon still owns the live process.
...
<Command
	name="superset agents sessions send <session-id> [prompt...]"
	args={[
		{ name: "session-id", required: true, description: "Full terminal session id." },
		{ name: "prompt", required: false, description: "Follow-up prompt text." },
	]}
	options={[
		{ flag: "--file <path>", description: "Read the prompt from a file; use - for stdin." },
		{ flag: "--wait", description: "Wait for the resulting turn to settle." },
		{ flag: "--timeout <duration>", description: "Maximum wait, such as 30s or 5m." },
		{ flag: "--host <id>", description: "Probe only one host." },
		{ flag: "--local", description: "Probe only this machine." },
	]}
	output={`Without --wait:
...
{
  terminalId: string;
  accepted: { accepted: true; sentAt: number };
  final: AgentSession | { status: "exited" };
  read: AgentSessionRead | null;
}`}
>
Send one semantic follow-up to a live agent. Choose exactly one prompt source:
positional text, `--file`, or piped stdin. Multiline input is submitted as one
bracketed paste and one submit key. `--wait` ignores an idle state that predates
the send, then returns when the resulting turn becomes idle, requests
permission, fails, or exits. Its response includes a fresh snapshot unless the
process exited.
...
printf 'Check both files\nand run tests\n' | superset agents sessions send <session-id>
```
</Command>

<Command
	name="superset agents sessions wait <session-id>"
	args={[
		{ name: "session-id", required: true, description: "Full terminal session id." },
	]}
	options={[
		{ flag: "--for <status>", required: true, description: "working, permission, idle, or failed." },
		{ flag: "--timeout <duration>", description: "Maximum wait (default 5m)." },
		{ flag: "--host <id>", description: "Probe only one host." },
		{ flag: "--local", description: "Probe only this machine." },
	]}
	output={`AgentSession | { status: "exited" }`}
>
Poll a live agent until it reaches the requested state, exits, disappears, the
timeout expires, or the command is interrupted. `--timeout` accepts milliseconds,
seconds, minutes, or hours, such as `250ms`, `30s`, `5m`, or `1h`.
...
All four session commands operate through the host service and PTY daemon; the
desktop renderer does not need to be open. They address only a currently live
process. Resuming an exited native agent session is agent-specific and is not
