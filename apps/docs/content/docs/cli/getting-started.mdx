...
Run `superset` with no arguments in a terminal to open an interactive
command browser: arrow-key through the command tree, read each command's
flags, and fill in required inputs with guided prompts. The confirmation
screen shows the exact assembled command before running it, so the browser
doubles as a way to learn the syntax. `superset --help` prints the same
catalog as a static, grouped help screen. Agent and CI environments always
get static help and JSON output — the browser never opens there.
