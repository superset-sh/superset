...
Every command targets exactly one host. Reads (`workspaces list`,
`projects list`) default to this machine; mutating host commands
(`projects create`, `projects setup`, `workspaces create`) require an
explicit `--host` or `--local` — there's no default. There is no
org-wide listing; the desktop app is the cross-host view.
