...
|---|---|---|
| **What it does** | Runs multiple agents in parallel with Git worktree isolation, you direct the work | Coordinator agent drafts a spec, decomposes tasks, runs implementor agents in waves, verifier checks results |
...
Yes. Intent backs each workspace with an isolated worktree and a dedicated branch, and each implementor commits to its own branch during parallel waves. Superset creates a worktree for each new local branch-based workspace. The isolation mechanism is the same; the orchestration philosophy differs.
