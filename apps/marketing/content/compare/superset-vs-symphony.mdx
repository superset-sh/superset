...
| | **Superset** | **Symphony** |
|---|---|---|
| **What it does** | Runs multiple AI agents in parallel with Git worktree isolation, chat, review, and browser | Turns tracker tickets into isolated, autonomous Codex implementation runs |
...
| **Isolation** | Git worktrees for local branch-based workspaces on your machines | Git clone per issue in a contained workspace root |
...
No -- it clones the repo into an isolated workspace per issue, contained within a workspace root. Superset uses a Git worktree for each new local branch-based workspace, which shares the repository's object store and is cheaper to create.
