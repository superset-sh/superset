...
|---|---|---|
| **What it does** | Runs multiple agents in parallel with Git worktree isolation, plus review, automations, and orchestration APIs | Dispatches agents into worktrees with diff review and one-key merge back to main |
...
Essentially yes. Both support worktree-backed tasks. Superset creates a branch and worktree for each new local branch-based workspace; worktrees do not sandbox processes or prevent merge conflicts. Parallel Code also symlinks `node_modules` and similar directories into new worktrees; Superset manages worktree lifecycle as part of its workspace model.
