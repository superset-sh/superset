...
| **Automations** | Scheduled recurring agent runs built in | Not offered |
| **Programmatic control** | MCP server, CLI, TypeScript SDK | Not offered |
...
| **Maker** | Superset (San Francisco company, three ex YC CTO founders) | Johannes Millan, solo open-source maintainer |
...
Parallel Code does one loop exceptionally directly: task in, worktree up, agent runs, diff reviewed, branch merged. If that is the entire job, its focus is a feature. Superset treats the loop as the core of a larger workflow: persistent terminal sessions that survive restarts, port management for concurrent dev servers, an in-app browser, chat, and remote and cloud workspaces. The more your agent usage looks like infrastructure rather than an afternoon tool, the more that surface area matters.
...
Parallel Code has genuinely nice touches Superset does not: optional per-task Docker sandboxing (with the documented caveat that Antigravity CLI cannot authenticate inside Docker), the AI Arena race mode, and coverage badges in the changed-files panel. If hard sandboxing of agent execution is a requirement, Parallel Code addresses it locally today.

### Project Shape: Solo OSS vs Company

Parallel Code is MIT, free, and moving fast, and it is overwhelmingly a single-maintainer project. That cuts both ways: full code freedom and quick releases, but bus-factor risk and no support organization, team features, or enterprise story. Superset is source-available rather than OSI open source, built by a funded team, and carries team and enterprise surfaces. Which trade you prefer is a legitimate philosophical choice.
...
- Want sessions, automations, and fleet monitoring, not just per-task dispatch
- Need programmatic control (MCP server, CLI, SDK) or scheduled runs
- Are adopting parallel agents as a team, not just personally
...
**Verdict:** Parallel Code is one of the best pure implementations of the worktree-dispatch loop, and its MIT license makes it the obvious pick for open-source purists. Superset is the better choice when parallel agents are becoming how you and your team ship: the same loop, plus persistence, automations, orchestration APIs, and the surfaces that turn a workflow into infrastructure.
