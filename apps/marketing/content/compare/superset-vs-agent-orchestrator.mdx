...
|---|---|---|
| **What it does** | Runs multiple AI agents in parallel with Git worktree isolation, chat, review, and browser | Supervises parallel agents in worktrees with automatic CI/review/conflict feedback loops |
...
| **Isolation** | Git worktrees for local branch-based workspaces | Worktree per session (or full clone); tmux/process runtimes |
...
Yes. Superset creates a worktree for each new local branch-based workspace; AO creates one per session, with a full-clone option for tooling that dislikes shared `.git` directories.
