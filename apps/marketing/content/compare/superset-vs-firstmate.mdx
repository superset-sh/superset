...
| | **Superset** | **Firstmate** |
|---|---|---|
| **What it is** | Desktop workspace that runs multiple AI agents in parallel with worktree isolation, chat, review, and browser | An "agent distro": instructions, skills, and scripts that make one agent orchestrate a crew |
...
| **Isolation** | Git worktrees for local branch-based workspaces | Disposable Git worktree per crewmate |
...
Yes. Firstmate gives each crewmate a disposable worktree (or delegates worktree management to Orca when used as a backend). Superset creates a worktree for each new local branch-based workspace and attaches persistent terminals and diff review to it.
