...
| | **Superset** | **Mux** |
|---|---|---|
| **What it does** | Runs multiple AI agents in parallel with Git worktree isolation, chat, review, and browser | Plans and executes tasks with parallel sessions of its own agent loop |
...
| **Isolation** | Git worktrees for local branch-based workspaces | Six runtimes: local, worktree, SSH, Docker, devcontainer, Coder |
...
Both offer them. Worktree is one of Mux's six runtimes alongside local, SSH, Docker, devcontainer, and Coder workspaces. Superset creates worktrees for new local branch-based workspaces; cloud workspaces use clones and projectless sessions use scratch folders.
