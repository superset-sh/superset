...
|---|---|---|
| **What it does** | Runs multiple agents in parallel with Git worktree isolation, review, browser, and automation | Runs Claude Code and Codex sessions in parallel Git worktrees with review and Git ops |
| **Agent support** | Any CLI agent (Claude Code, Codex, OpenCode, Cursor, Gemini, Mistral Vibe, and more) | Claude Code and Codex |
| **Isolation** | Git worktrees for local branch-based workspaces | Git worktree per session |
...
| **Platform** | Desktop (macOS; experimental Linux AppImage; Windows unavailable), CLI, MCP server | Desktop (macOS, Windows, Linux) |
