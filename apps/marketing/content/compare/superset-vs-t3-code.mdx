...
| | **Superset** | **T3 Code** |
|---|---|---|
| **What it does** | Runs multiple agents in parallel with Git worktree isolation, review, browser, and automation | A control plane that runs coding agents, each thread on its own branch/worktree |
| **Isolation** | Git worktrees for local branch-based workspaces | Git worktree per agent thread (worktree/local toggle) |
...
| **Platform** | Desktop (macOS; experimental Linux AppImage; Windows unavailable), CLI, MCP server | Desktop (macOS, Windows, Linux) plus `npx` launcher |
...
Yes. Both support worktree-backed tasks; Superset creates worktrees for new local branch-based workspaces (T3 Code offers a per-thread worktree-or-local toggle). The isolation model is shared; the products differ in maturity and surrounding features.
