...
| **Parallel agents** | Yes, worktree per task | Yes, up to 8 in local worktrees | Yes, separate local branch-based worktrees |
...
Yes. Conductor uses a worktree per task, and Cursor 2.0 runs its parallel agents in local Git worktrees too. Superset uses Git worktrees for new local branch-based workspaces.
