...
If you want that local orchestration but agent-agnostic on macOS and experimental Linux, [Superset](/compare/superset-vs-codex) is a third option. This page compares all three.
...
| **Parallelism** | Worktree per task (local) | Isolated cloud environments | Local branch-based worktrees; connected hosts |
| **Other agents** | Claude Code, Cursor | OpenAI Codex only | Claude Code, OpenCode, Cursor, Gemini, and more |
| **Platform** | macOS only | CLI, editor, desktop, web, cloud | macOS; experimental Linux AppImage; Windows unavailable |
...
**Verdict:** Codex is the agent; Conductor is a Mac app that runs it (and others) in local worktrees. Choose Codex for OpenAI's cloud-first suite, Conductor for a focused local Mac orchestrator, and Superset if you want CLI-agent orchestration on macOS and experimental Linux.
