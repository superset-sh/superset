...
Superset runs entirely locally — no telemetry, code stays on your machine. Copilot sends code context to GitHub/Microsoft servers for processing. Copilot Business and Enterprise offer data retention controls, but code still transits external servers. Superset's source-available codebase is fully auditable.
...
No. Superset is source-available on GitHub under Elastic License 2.0 (ELv2) with zero telemetry. GitHub Copilot is closed source.
