...
| | **Superset** | **Gas Town** |
|---|---|---|
| **What it does** | Runs multiple AI agents in parallel with Git worktree isolation, chat, review, and browser | Coordinates fleets of 20-30 agents with an AI coordinator, work ledger, and merge queue |
...
| **Isolation** | Git worktrees for local branch-based workspaces | Git worktree-backed "hooks" persisting agent work |
...
Yes. Superset creates a worktree for each new local branch-based workspace. Gas Town's "hooks" persist each agent's work in git worktree-backed storage so it survives crashes and restarts.
