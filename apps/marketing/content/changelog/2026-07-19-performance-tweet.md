...
We also cut background CPU from port detection: one shared process scan per cycle instead of one per terminal. With 15 terminals open that dropped background ps spawns from 90 to 6 over the same window, and the gap grows the more terminals you have.

Every number above was measured on the real code, not estimated.
