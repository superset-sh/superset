...
You can imagine the agent loop as a pipeline:

[Insert an image of the pipeline, You come up with an idea → You plan the task → You hand it to an agent → Agent works →
You review → You give feedback → Agent revises → You review again → You merge]

A human is involved in almost every step, and each of these steps has a steep context-switching cost - you have to open
that agent's code, spin up dev servers, click through the UI to verify their work, give feedback and more. Right now,
most of our agents spend more time waiting for us to review their work than they spend doing it.
...
agent-driven UI's - interfaces that agents orchestrate for the human's benefit, where each review takes seconds, not
minutes.
...
"create PR" or "deploy to staging" should also be easy for you to reach to.
...
per agent interaction? If you're running agents at scale and hitting these walls, we'd love to compare notes.
