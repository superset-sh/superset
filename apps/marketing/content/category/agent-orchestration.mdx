...
Agent orchestration is everything around the coding agents: creating isolated workspaces, launching agents into them, keeping sessions alive, watching status, reviewing output, scheduling recurring runs, and letting programs (including other agents) do all of the above without a human clicking. The agents write the code. Orchestration decides where, when, and under what supervision they run.
...
The test is simple: if your agents stop the moment you close your laptop lid, you have sessions, not orchestration.

## The primitives

Every serious orchestration setup, homegrown or product, ends up with the same six pieces.
...
A fleet view: which agents are running, which are waiting for input, which finished, what changed. This is the piece that turns "check every tab in rotation" into "get pulled in only when needed", and it matters more than any other UI decision in the category.
...
Neither is strictly better. Coordinator-led shines on PR-sized features with clear requirements; developer-led wins when task boundaries need judgment, which, in most real codebases, is most of the time.

## When you don't need orchestration

Honest scoping saves you a tool migration later:
