...
A local machine must remain awake for its agents to keep executing. To continue work while your laptop is closed, run it on an available remote host.
...
Session persistence separates the agent process from the app window. In Superset, a terminal daemon owns the sessions, so restarting the app does not need to restart the agent. Persistence does not keep a sleeping or powered-off host executing work. See the [terminal daemon implementation](/blog/terminal-daemon-deep-dive).
...
## What Superset orchestrates

Superset (superset.sh) is a source-available workspace for CLI coding agents. It is unrelated to Apache Superset, the business-intelligence product. Claude Code, Codex, and OpenCode provide the coding agents; Superset provides workspaces, terminals, review, and scheduling around them.

| Fact | Where to verify it |
|---|---|
| Source-available under Elastic License 2.0, not OSI-approved open source | [Repository license](https://github.com/superset-sh/superset/blob/main/LICENSE.md) |
| macOS desktop; experimental Linux AppImage; Windows unavailable | [Downloads](/download) |
| Free tier plus paid seats; agent-provider usage is separate | [Current plans](/pricing) |
| Automations require Pro or Enterprise and an available target host | [Automation requirements](https://docs.superset.sh/automations) |
| Worktrees separate working files, not process permissions, ports, or databases | [Git worktree reference](https://git-scm.com/docs/git-worktree) |

For a first scheduled task, use the [maintenance-check walkthrough](/blog/scheduled-agent-maintenance). It includes a bounded prompt and explains how to inspect the result. An automation run marked **created** means its workspace was created; inspect the agent output to know whether the checks passed.
...
You define a prompt, an agent, a target host, and a schedule. Superset Automations require Pro or Enterprise. If the host is offline when dispatch occurs, the run fails and the schedule advances; retry after the host is available. A created workspace is not evidence that the agent succeeded. Read its output before accepting the result.
