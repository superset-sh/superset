		...
		>
			...
			<div className="bg-[#1a1a1a]/90 backdrop-blur-sm rounded-lg border border-white/10 shadow-2xl overflow-hidden">
				...
				<div className="px-3 py-3 border-t border-white/5">
					...
					>
						<button
							type="button"
							className="flex items-center gap-2 px-3 py-1.5 text-sm text-white/90 bg-[#2a2a2a] border border-white/10 rounded-l-md hover:bg-[#3a3a3a] transition-colors"
						>
							<Image
						...
						<button
							type="button"
							className="flex items-center gap-1 px-2 py-1.5 text-sm text-white/90 bg-[#2a2a2a] border border-white/10 border-l-0 rounded-r-md hover:bg-[#3a3a3a] transition-colors"
						>
							<HiChevronDown className="w-3.5 h-3.5 rotate-180" />
						</button>
					</motion.div>
				</div>
			</div>

			{/* Dropdown Menu - flows outside the window */}
			<motion.div
				className="absolute left-3 -bottom-2 translate-y-full w-44 bg-[#1e1e1e] border border-white/10 rounded-md shadow-2xl overflow-hidden"
			...
			>
				{IDE_OPTIONS.map((ide, index) => (
					<motion.div
						key={ide.id}
						className="flex items-center justify-between px-3 py-2 text-sm text-white/80 hover:bg-white/5 cursor-pointer transition-colors"
					...
					>
						...
						{ide.shortcut && (
							<span className="text-xs text-white/40">{ide.shortcut}</span>
						)}
					</motion.div>
				))}
			</motion.div>
