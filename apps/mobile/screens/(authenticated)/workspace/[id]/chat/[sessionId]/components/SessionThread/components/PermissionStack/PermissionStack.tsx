import {
	makeSelectedOutcome,
	type TimelinePendingPermission,
...
export function PermissionStack({
	...
	const visible = pending.filter(
		(entry) => !answeredIds.has(entry.permissionId),
	);
	const front = visible[0];
	if (!front) return null;

	// Multi-select cards answer with every picked option; single-select passes
