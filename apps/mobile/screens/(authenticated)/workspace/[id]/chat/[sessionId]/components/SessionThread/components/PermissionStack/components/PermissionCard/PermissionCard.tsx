...
import type { TimelinePendingPermission } from "@superset/host-service-sync/timeline";
...
export function PermissionCard({
	...
	onAnswer: (permissionId: string, optionIds: string[]) => void;
}) {
	...
	const title = pending.toolCall.title || "Permission";
	...
	const handleOption = (option: PermissionOption) => {
		if (!multiSelect || isReject(option)) {
			onAnswer(pending.permissionId, [option.id]);
			return;
		}
		setPicked((prev) => {
			...
			<View className="border-border border-t px-4 py-1">
				...
				{multiSelect ? (
					<Pressable
						...
						onPress={() => onAnswer(pending.permissionId, [...picked])}
					>
						<Text
							className={cn(
								"text-sm font-medium",
								...
								<View className="px-5 pt-6 pb-8">
									...
									>
										<View className="gap-4">
											...
											{pending.toolCall.input === null ? null : (
												<ToolInput input={pending.toolCall.input} />
											)}
										</View>
									</ScrollView>
								</View>
