...
import type { TimelineMessageItem } from "@superset/host-service-sync/timeline";
...
function blocksToText(blocks: ContentBlock[]): string {
	return blocks
		.map((block) => {
			if (block.type === "text" || block.type === "thought") return block.text;
 ...
 */
export function MessageItemView({ item }: { item: TimelineMessageItem }) {
	...
	if (item.role === "user") {
		// A failed prompt (the turn carrying this message failed after it was
