...
const reviewNotes = [
	"WHAT THE APP IS",
	"Superset Mobile is the companion app for Superset (https://superset.sh), a desktop tool where developers run AI coding agents on their own computers or in Superset cloud workspaces. The phone app is a remote client for those sessions, in the same class as an SSH or remote desktop client: it shows agent progress, lets the user chat with an agent, review file diffs, and type into a terminal session that is running on the user's own machine. No code is downloaded, installed, or executed on the device; everything shown is streamed from the user's host over the network.",
	"",
	"HOW TO REVIEW",
