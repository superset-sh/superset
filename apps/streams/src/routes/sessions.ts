...
export function createSessionRoutes(protocol: AIDBSessionProtocol) {
	...
	app.get("/:sessionId", async (c) => {
		...
		try {
			...
			return c.json(
				{ error: "Failed to get session", code: "GET_FAILED", sessionId, details: (error as Error).message },
				500,
			);
		}
	});
	...
	app.post("/:sessionId/reset", async (c) => {
		...
		try {
			...
			return c.json(
				{ error: "Failed to reset session", code: "RESET_FAILED", sessionId, details: (error as Error).message },
				500,
			);
		}
	});
