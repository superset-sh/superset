...
export function RunwayTile() {
	...
	const description = available
		? `$${(data.totalCashUsd / 1_000_000).toFixed(2)}M cash ÷ $${data.avgMonthlyNetBurnUsd?.toLocaleString() ?? "—"}/mo net burn, last 3 complete months. Cash includes raised capital; no growth assumed.`
		: ((data && !data.available ? data.reason : null) ??
			"Cash ÷ net burn at the current run rate");
