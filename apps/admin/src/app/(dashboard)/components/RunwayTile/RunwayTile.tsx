...
export function RunwayTile() {
	...
	const description = available
		? t({
				message: `$${(data.totalCashUsd / 1_000_000).toFixed(2)}M cash ÷ $${data.avgMonthlyNetBurnUsd == null ? "—" : formatNumber(data.avgMonthlyNetBurnUsd)}/mo net burn, last 3 complete months. Cash includes raised capital; no growth assumed.`,
			})
	...
	return (
		<MetricCard
			...
			error={query.error}
			formatter={(v) => t({ message: `${formatNumber(v, undefined)} mo` })}
		/>
	);
}
