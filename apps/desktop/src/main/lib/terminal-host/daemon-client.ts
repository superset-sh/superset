/**
 * Generic terminal daemon client.
 *
 * The app-facing terminal client and the supervisor's worker client both use
 * this implementation. The runtime paths and spawn target are injected so the
 * same request/stream protocol can be reused across layers.
 */
...
import type { TerminalDaemonRuntimePaths } from "./runtime-paths";
import {
		...
		while (newlineIndex !== -1) {
			...
			if (line.trim()) {
				try {
					...
					console.warn("[TerminalDaemonClient] Failed to parse NDJSON line");
				}
			}
...
export interface TerminalDaemonClientEvents {
...
/**
 * Runtime configuration for a daemon instance.
 */
...
export function resolveDaemonScriptPath({
	...
	sourceRelativePath,
	bundledRelativePath,
}: ResolveDaemonScriptPathOptions): string {
	const sourcePath = resolve(moduleDir, sourceRelativePath);
	if (existsSync(sourcePath)) {
		return sourcePath;
	}

	return resolve(moduleDir, bundledRelativePath);
}

/**
 * Client for communicating with a terminal daemon-compatible process.
...
export class TerminalDaemonClient extends EventEmitter {
	...
	private debugLog(message: string, data?: unknown): void {
		...
		const prefix = `[TerminalDaemonClient:${this.daemonName}]`;
		if (data === undefined) {
	...
	async ensureConnected(): Promise<void> {
		...
		this.debugLog("Connecting to daemon...");
		...
		for (let attempt = 0; attempt < 2; attempt++) {
			if (attempt === 0 && process.env.NODE_ENV === "development") {
				if (this.isDaemonScriptStale()) {
					this.debugLog("Daemon script rebuilt, restarting...");
			...
			try {
				...
				if (attempt === 0) {
					this.debugLog("Auth token missing, restarting daemon...");
					this.resetConnectionState({ emitDisconnected: false });
					this.killDaemonFromPidFile();
					await this.waitForDaemonShutdown();
					await this.spawnDaemon();
			...
			try {
				...
				if (
					attempt === 0 &&
					this.supportsLegacyProtocolDowngrade &&
					this.isProtocolMismatchError(error)
				) {
					this.debugLog(
						"Protocol mismatch detected, shutting down legacy daemon...",
					);
					this.resetConnectionState({ emitDisconnected: false });
	...
	private isDaemonScriptStale(): boolean {
		try {
			if (!existsSync(this.runtimePaths.scriptMtimePath)) {
				return false; // No mtime file = first run or manual cleanup
			}

			const savedMtime = readFileSync(
				this.runtimePaths.scriptMtimePath,
				"utf-8",
			).trim();
	...
	private saveDaemonScriptMtime(): void {
		try {
			...
			writeFileSync(this.runtimePaths.scriptMtimePath, mtime, {
	...
	private killDaemonFromPidFile(): void {
		if (!existsSync(this.runtimePaths.pidPath)) return;

		try {
			const raw = readFileSync(this.runtimePaths.pidPath, "utf-8").trim();
	...
	private async tryConnectControl(): Promise<boolean> {
		return new Promise((resolve) => {
			if (!existsSync(this.runtimePaths.socketPath)) {
				resolve(false);
				return;
			}

			const socket = connect(this.runtimePaths.socketPath);
			let resolved = false;
	...
	private readAuthToken(): string {
		if (!existsSync(this.runtimePaths.tokenPath)) {
			throw new Error("Auth token not found - daemon may not be running");
		}

		return readFileSync(this.runtimePaths.tokenPath, "utf-8").trim();
	}
	...
	private async shutdownLegacyDaemon({
		...
		if (!existsSync(this.runtimePaths.socketPath)) return;
	...
	private isSocketLive(): Promise<boolean> {
		return new Promise((resolve) => {
			...
			const testSocket = connect(this.runtimePaths.socketPath);
			const timeout = setTimeout(() => {
				testSocket.destroy();
				resolve(false);
			}, 1000);
	...
	private acquireSpawnLock(): boolean {
		try {
			...
			if (existsSync(this.runtimePaths.spawnLockPath)) {
				const lockContent = readFileSync(
					this.runtimePaths.spawnLockPath,
					"utf-8",
				).trim();
				...
				unlinkSync(this.runtimePaths.spawnLockPath);
			}

			// Create lock file with current timestamp
			writeFileSync(this.runtimePaths.spawnLockPath, String(Date.now()), {
				mode: SUPERSET_SENSITIVE_FILE_MODE,
			});
			return true;
		} catch {
	...
	private async spawnDaemon(): Promise<void> {
		...
		if (existsSync(this.runtimePaths.socketPath)) {
			...
			if (isLive) {
				this.debugLog("Socket is live, daemon is running");
				return;
			}

			// Socket exists but not responsive - safe to remove
			this.debugLog("Removing stale socket file");
			try {
				unlinkSync(this.runtimePaths.socketPath);
			} catch {
		...
		if (existsSync(this.runtimePaths.pidPath)) {
			this.debugLog("Removing stale PID file");
			try {
				unlinkSync(this.runtimePaths.pidPath);
			} catch {
				// Ignore - might not have permission
			}
		}
		...
		if (!this.acquireSpawnLock()) {
			this.debugLog("Another spawn in progress, waiting...");
		...
		try {
			...
			this.debugLog("Daemon script path", daemonScript);
			this.debugLog("Script exists", existsSync(daemonScript));

			if (!existsSync(daemonScript)) {
				throw new Error(`Daemon script not found: ${daemonScript}`);
			}

			this.debugLog("Spawning daemon", {
				execPath: process.execPath,
				daemonScript,
			});

			// Open log file for daemon output (helps debug daemon-side issues)
			const logPath = this.runtimePaths.logPath;
			let logFd: number;
			try {
				...
				logFd = openSync(logPath, "a", SUPERSET_SENSITIVE_FILE_MODE);
				try {
					chmodSync(logPath, SUPERSET_SENSITIVE_FILE_MODE);
				} catch {
					// Best-effort.
				}
			} catch (error) {
				console.warn(
					`[TerminalDaemonClient:${this.daemonName}] Failed to open daemon log file: ${error}`,
				);
			...
			this.debugLog("Daemon spawned", { pid: child.pid });

			// Unref to allow parent to exit independently
			child.unref();

			// Wait for daemon to start
			this.debugLog("Waiting for daemon to start...");
			await this.waitForDaemon();
			...
			this.debugLog("Daemon started successfully");
		} finally {
	 ...
	 * After calling this, the client should be disposed before reuse.
	 */
