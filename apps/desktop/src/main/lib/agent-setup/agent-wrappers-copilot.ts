...
# v2 host-service terminals only set SUPERSET_TERMINAL_ID; v1 still sets SUPERSET_TAB_ID.
if { [ -n "$SUPERSET_TAB_ID" ] || [ -n "$SUPERSET_TERMINAL_ID" ]; } && [ -f "${hookScriptPath}" ]; then
