...
export function requestLocalNetworkAccess(): void {
	...
	socket.on("error", (err) => {
		console.log("[local-network] Socket error (expected if permission denied):", err.message);
		socket.close();
	});
	...
	socket.bind(() => {
		try {
			...
			socket.send(message, 0, message.length, MDNS_PORT, MDNS_MULTICAST_ADDRESS, (err) => {
				if (err) {
					console.log("[local-network] Send error (expected if permission denied):", err.message);
				} else {
