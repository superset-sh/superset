...
describe("Session shell-ready: write buffering", () => {
	...
	it("flushes buffered writes on timeout", async () => {
		const { session, proc } = createTestSession("/bin/zsh");
		spawnAndReady(session, proc);
...
describe("Session shell-ready: marker detection", () => {
	let _broadcastedData: string[];

	function createTrackedSession(shell: string) {
		const result = createTestSession(shell);
		_broadcastedData = [];
		return result;
	}

	it("strips marker from single data frame", () => {
		const { session, proc } = createTrackedSession("/bin/zsh");
		spawnAndReady(session, proc);
