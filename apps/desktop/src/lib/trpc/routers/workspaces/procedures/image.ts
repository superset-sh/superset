...
/**
 * Discover images in a workspace repository using git ls-files
 */
async function discoverImagesInRepo(worktreePath: string): Promise<string[]> {
	try {
...
/**
 * Find the best auto-detected image for a workspace
 */
async function findAutoImage(worktreePath: string): Promise<string | null> {
	const images = await discoverImagesInRepo(worktreePath);
	...
	for (const imagePath of images) {
		...
		try {
			const stats = await secureFs.stat(worktreePath, imagePath);
...
export const createImageProcedures = () => {
	return router({
		/**
		 * Discover candidate images in a workspace repository
		 */
		discoverImages: publicProcedure
			...
			.query(async ({ input }) => {
				...
				const worktreePath = getWorkspacePath(workspace);
				if (!worktreePath) {
					throw new TRPCError({
						code: "INTERNAL_SERVER_ERROR",
						message: "Cannot determine workspace path",
					});
				}
		...
		/**
		 * Get the image data for a workspace thumbnail
		 */
		getWorkspaceImage: publicProcedure
			.input(z.object({ workspaceId: z.string() }))
			.query(async ({ input }) => {
				const workspace = getWorkspaceNotDeleting(input.workspaceId);
				if (!workspace) {
				...
				// Check if workspace has a custom image set
				...
				if (imagePath === null) {
					const autoPath = await findAutoImage(worktreePath);
					if (!autoPath) {
						return { type: "fallback" as const };
					}
				...
				try {
					const buffer = await secureFs.readFileBuffer(worktreePath, imagePath);
		...
		/**
		 * Set the image path for a workspace
		 */
		setWorkspaceImage: publicProcedure
			.input(
				z.object({
					workspaceId: z.string(),
			...
			.mutation(async ({ input }) => {
				...
				if (input.imagePath && input.imagePath !== "") {
					...
					try {
						const stats = await secureFs.stat(worktreePath, input.imagePath);
						if (stats.size > MAX_IMAGE_SIZE_BYTES) {
							throw new TRPCError({
								code: "BAD_REQUEST",
