...
export async function getProcessEnvWithShellPath(
	...
	// Git ≥ 2.50 refuses to honor *inherited* `PAGER`/`GIT_PAGER` or
	// `EDITOR`/`GIT_EDITOR` env vars on non-interactive callers
	...
	// which simple-git surfaces as GitPluginError and breaks workspace
	// creation, status reads, worktree prune, etc.
	//
	// Strip them all. Programmatic git invocations have piped stdout (not a
	// TTY) so paging is skipped, and they never prompt for commit messages
	// or hunk edits, so no replacement value is needed. User-facing
	// terminals get their own PAGER/EDITOR from the interactive shell, so
	// this only affects the host-service / desktop main process's own git
	// children.
	delete env.PAGER;
