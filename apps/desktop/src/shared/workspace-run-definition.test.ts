...
describe("selectWorkspaceRunDefinition", () => {
	...
	it("ignores non-string config commands instead of throwing", () => {
		const definition = selectWorkspaceRunDefinition({
			projectId: "project-a",
			configRunCommands: [
				{ name: "Dev server", command: "bun dev" },
				"bun start",
			],
			presets: [],
		});
	...
	it("falls back when all config commands are non-strings", () => {
		const definition = selectWorkspaceRunDefinition({
			projectId: "project-a",
			configRunCommands: [{ name: "Dev server", command: "bun dev" }],
			presets: [
				{
					id: "preset-global",
					name: "Global dev",
	...
	it("ignores non-string preset commands instead of throwing", () => {
		const definition = selectWorkspaceRunDefinition({
			projectId: "project-a",
			presets: [
				{
					id: "preset-global",
					name: "Global dev",
					commands: [{ name: "Dev server" }, "npm run dev"],
					useAsWorkspaceRun: true,
				},
			],
		});
