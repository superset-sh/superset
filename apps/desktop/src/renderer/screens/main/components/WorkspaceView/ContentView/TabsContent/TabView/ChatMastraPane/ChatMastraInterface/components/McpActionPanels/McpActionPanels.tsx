...
interface McpActionPanelsProps {
	...
	activeToolEntries: ActiveToolEntry[];
	...
	return (
		<div className="mx-auto flex w-full max-w-[680px] flex-col gap-2 px-4 pb-2">
			...
			{activeToolEntries.length > 0 && (
				<div className="rounded-md border border-border bg-muted/20 px-3 py-2">
					<div className="text-xs font-medium text-foreground">
						Active Mastra/MCP tools ({activeToolEntries.length})
					</div>
					<div className="mt-2 space-y-1">
						{activeToolEntries.slice(0, 5).map(({ id, tool }) => (
							<div
								key={id}
								className="flex items-center justify-between gap-2 rounded border border-border/50 bg-background/70 px-2 py-1 text-xs"
							>
								<div className="min-w-0 truncate font-medium text-foreground">
									{tool.name}
								</div>
								<div className="truncate text-muted-foreground">
									{truncate(stringifyCompact(tool.args), 100)}
								</div>
								<div className="shrink-0 rounded border border-border bg-muted px-1.5 py-0.5 uppercase tracking-wide text-[10px] text-muted-foreground">
