...
export function useV2SidebarData() {
	...
	const { toggleProjectCollapsed } = useV2SidebarState();
	...
	const groups = useMemo<V2SidebarProject[]>(() => {
		...
		const localSectionsByProject = new Map<string, V2SidebarSection[]>();
		...
		const workspaceRowsByProject = new Map<string, V2SidebarWorkspace[]>();
		const workspaceRowsBySection = new Map<string, V2SidebarWorkspace[]>();

		for (const localWorkspace of sidebarWorkspaces) {
			const workspace = cloudWorkspacesById.get(localWorkspace.workspaceId);
			if (!workspace) continue;

			const sidebarWorkspace: V2SidebarWorkspace = {
				id: workspace.id,
		...
		const resolvedProjects: V2SidebarProject[] = [];
