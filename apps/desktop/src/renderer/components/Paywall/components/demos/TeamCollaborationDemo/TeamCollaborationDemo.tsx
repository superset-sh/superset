...
const TEAM_MEMBERS = [
	...
	{ name: "Alex Rivera", initials: "AR", color: "bg-purple-500" },
	{ name: "Jordan Lee", initials: "JL", color: "bg-emerald-500" },
	{ name: "Taylor Kim", initials: "TK", color: "bg-amber-500" },
];
		...
		<div className="w-full max-w-[280px] bg-[#1a1a1a]/90 backdrop-blur-sm rounded-lg border border-white/10 shadow-2xl overflow-hidden">
			...
			<div className="flex items-center justify-between px-4 py-3 bg-[#2a2a2a]/80 border-b border-white/5">
				<div className="flex items-center gap-2">
					...
					<span className="text-xs text-white/60 ml-1">Team</span>
				</div>
			</div>

			{/* Team members */}
			<div className="p-4">
				<div className="text-[10px] uppercase text-white/40 font-medium tracking-wider mb-3">
					Online Now
				</div>
				<div className="flex items-center -space-x-2 mb-4">
					{TEAM_MEMBERS.map((member, index) => (
						<div
							key={member.name}
							className={cn(
								"w-9 h-9 rounded-full flex items-center justify-center text-xs font-medium text-white border-2 border-[#1a1a1a]",
								member.color,
							)}
				...
				<div className="space-y-2">
					{ACTIVITY.map((item) => (
						<div key={item.id} className="flex items-center gap-2 text-xs">
							...
							<span className="text-white/30 ml-auto">{item.time}</span>
						</div>
					))}
				</div>
			</div>
