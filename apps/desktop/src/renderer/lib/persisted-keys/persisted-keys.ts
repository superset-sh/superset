 ...
 * feature stops writing a key the key moves to DEAD_KEYS below — otherwise it
 * survives on user profiles forever (a 2026-07 audit found 53 KB of
 * `pending-workspaces-*` records unread since May, plus seven other dead
 * families). The registry-enforcement test fails when a file writes
 * localStorage without an entry here.
 */

export interface PersistedKeyOwner {
...
/**
 * Keys written into the app origin by third-party libraries we ship. Not
 * swept — the library manages them — but listed so an audit can attribute
 * them and the enforcement test doesn't demand a registry owner.
 */
export const LIBRARY_OWNED_KEY_PREFIXES = [
	"ph_", // posthog-js
	"__ph_opt_in_out_", // posthog-js consent
