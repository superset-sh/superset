...
function createFakeRuntime(terminalId: string): FakeRuntime {
	...
	return {
		...
		lastCols: 120,
		lastRows: 32,
		_enableImageAddon: null,
		_disableImageAddon: null,
		_enableWebglAddon: null,
...
describe("terminalRuntimeRegistry", () => {
	...
	it("does not probe unmarked canvases during stress WebGL loss", () => {
		const unmarkedCanvas = createCanvas({
		...
		runtime.wrapper._canvasList = [unmarkedCanvas];
