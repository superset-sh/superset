...
const INTRO =
	"Help me create and publish a Superset Page using the Superset Pages skill.";
...
const BUILD_AND_PUBLISH = `Before building, run superset pages --help and superset auth whoami to check that the CLI supports Pages and is authenticated. Follow the skill's content policy and design guidance. Keep the source inside this workspace and verify it renders correctly.
...
export function buildPageAgentPrompt(request?: string): string {
	const trimmed = request?.trim();
	const brief = trimmed
		? `This is what the page should cover:\n\n${trimmed}\n\nUse this workspace as the source material. Ask me only if something essential is missing.`
		: ASK_WHAT_TO_CREATE;
	return [INTRO, LOAD_SKILL, brief, BUILD_AND_PUBLISH].join("\n\n");
}
