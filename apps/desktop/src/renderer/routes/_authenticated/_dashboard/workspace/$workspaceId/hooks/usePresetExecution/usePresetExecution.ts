...
import { getPresetLaunchPlan } from "renderer/lib/preset-launch/preset-launch";
...
function resolveTarget(executionMode: TerminalPresetRow["executionMode"]) {
...
export function usePresetExecution({
	store,
	launcher,
	...
	// Read host agent configs from the host service — same data source as the
	...
	// `useAgentConfigs` is the cached source of truth for agent configs
	...
	// previous consumer (`useWorkspaceRun`) into a re-render cycle.
	const resolvePresetCommands = useCallback(
		(preset: TerminalPresetRow): string[] =>
	...
	const executePreset = useCallback(
		async (
			...
			try {
				...
				console.error("[usePresetExecution] Failed to execute preset:", err);
				toast.error(
					t({
