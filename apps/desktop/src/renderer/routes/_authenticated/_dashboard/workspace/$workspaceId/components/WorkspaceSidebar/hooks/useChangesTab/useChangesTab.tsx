...
import { useChangeset } from "renderer/routes/_authenticated/_dashboard/workspace/$workspaceId/hooks/useChangeset";
...
import { useSidebarDiffRef } from "renderer/routes/_authenticated/_dashboard/workspace/$workspaceId/hooks/useSidebarDiffRef";
...
export function useChangesTab({
	...
	const localState = collections.workspaceLocalState.get(workspaceId);
	const filter: ChangesFilter = localState?.sidebarState?.changesFilter ?? {
		kind: "all",
	};
	const { preferences, setChangesViewMode: setViewMode } = useUserPreferences();
