...
import type { usePresetExecution } from "../usePresetExecution";

type PresetExecution = ReturnType<typeof usePresetExecution>;
...
export function resolvePendingCreationPresets(
	...
	getPreset: (id: string) => TerminalPresetRow | undefined,
	resolvePresetCommands: (preset: TerminalPresetRow) => string[],
): TerminalPresetRow[] {
...
export function useRunWorkspaceCreationPresets({
	...
	useEffect(() => {
		...
		const presets = resolvePendingCreationPresets(
			presetIds,
			(id) => collections.terminalPresets.get(id),
			resolvePresetCommands,
		);
		void (async () => {
