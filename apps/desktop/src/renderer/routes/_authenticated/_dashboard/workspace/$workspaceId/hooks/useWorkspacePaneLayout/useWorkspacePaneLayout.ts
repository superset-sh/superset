...
} from "renderer/stores/pane-selection";
...
export function useWorkspacePaneLayout() {
	...
	const workspaceRuntime = useMemo(() => {
		const persistedLayout =
			(collections.workspaceLocalState.get(workspaceId)?.paneLayout as
				| WorkspaceState<PaneViewerData>
				| undefined) ?? EMPTY_STATE;
		const seededLayout = applyRememberedPaneSelection(
			workspaceId,
			persistedLayout,
		);
		return {
	...
	useEffect(() => {
		const unsubscribe = store.subscribe((nextStore) => {
			...
			rememberPaneSelection(workspaceId, nextWorkspaceState);
