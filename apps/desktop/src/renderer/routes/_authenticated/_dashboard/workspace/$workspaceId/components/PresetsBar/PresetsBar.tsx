...
interface PresetsBarProps {
	matchedPresets: TerminalPresetRow[];
	executePreset: (preset: TerminalPresetRow) => void | Promise<void>;
	showPresetsBar: boolean;
	onToggleShowPresetsBar: (enabled: boolean) => void;
}

// Co-located. Mirrors the array in
...
function isPresetVisibleInBar(pinnedToBar: boolean | undefined): boolean {
	// The persisted field is legacy "pinned" wording; the UI treats it as
...
export function PresetsBar({
	...
	const { setBuiltinPresetHidden } = useUserPreferences();
	...
	const handlePersistVisibleReorder = useCallback(
		...
		[collections.terminalPresets, localVisiblePresetIds, matchedPresets],
	);

	const handleTogglePresetVisibility = useCallback(
		(presetId: string, nextVisible: boolean) => {
			collections.terminalPresets.update(presetId, (draft) => {
			...
			<DropdownMenu>
				...
				<DropdownMenuContent align="end" className="w-56">
					{matchedPresets.map((preset) => {
						const icon = resolvePresetIcon(preset, agents, isDark);
