...
describe("buildForkPayload", () => {
	...
	test("plumbs worktreeBaseDir through to host-service input (#3929)", () => {
		const pending = makePending();
		const payload = buildForkPayload(
			"pid",
			pending,
			undefined,
			"/Users/me/code/worktrees",
		);
		expect(payload.worktreeBaseDir).toBe("/Users/me/code/worktrees");
	});

	test("worktreeBaseDir is undefined when not provided", () => {
		const pending = makePending();
		const payload = buildForkPayload("pid", pending, undefined);
		expect(payload.worktreeBaseDir).toBeUndefined();
	});
});
