...
import { HiArrowRight, HiChevronDown, HiPaperAirplane, HiXMark } from "react-icons/hi2";
...
export function IssueDetailSidebar({
	...
	const { data: comments } = electronTrpc.settings.getOnedevIssueComments.useQuery(
		{ issueId: issue?.id ?? 0 },
	...
	const updateFields = electronTrpc.settings.updateOnedevIssueFields.useMutation({
	...
	const enabledAgentPresets = useMemo(() => getEnabledAgentConfigs(agentPresets), [agentPresets]);
	const agentConfigsById = useMemo(() => indexResolvedAgentConfigs(agentPresets), [agentPresets]);
	const fallbackAgentId = useMemo(() => getFallbackAgentId(agentPresets), [agentPresets]);
	const selectableAgents = useMemo(() => enabledAgentPresets.map((p) => p.id), [enabledAgentPresets]);
	...
	const selectedProject = recentProjects.find((p) => p.id === effectiveProjectId);
	...
	const onedevToken = onedevConfig?.accessToken ?? "";
	...
	const handleOpen = async () => {
		...
		if (selectedAgent !== "none" && !agentConfigsById.get(selectedAgent)?.enabled) {
...
When done: commit your changes. Do NOT push or create PRs.`;
		...
		try {
			...
			toast.success(result.wasExisting ? "Opened existing workspace" : "Workspace created");
		} catch (err) {
		...
		<div className="shrink-0 flex overflow-hidden" style={{ width }}>
			...
			<div className="flex items-center justify-between px-3 py-2 border-b border-border shrink-0">
				<div className="flex items-center gap-2 min-w-0">
					...
					<span className="text-xs font-mono text-muted-foreground">{String(slug)}</span>
				</div>
				<div className="flex items-center gap-1">
					<button
						...
						onClick={() => navigate({
							...
							params: { projectPath: encodeURIComponent(projectPath), issueNumber: String(issueNumber) },
						})}
			...
			<div className="flex-1 overflow-y-auto">
				...
				<div className="px-3 py-3 border-b border-border flex flex-col gap-3">
					<div className="flex items-center justify-between">
						...
						<select
							value={issue.state}
							onChange={(e) => updateState.mutate({ issueId: issue.id, state: e.target.value })}
					...
					<div className="flex items-center justify-between">
						...
						<select
							...
							onChange={(e) => updateFields.mutate({ issueId: issue.id, fields: { Type: e.target.value } })}
							className="h-6 text-xs rounded border bg-transparent px-1 w-32"
						>
					...
					<div className="flex items-center justify-between">
						...
						<select
							...
							onChange={(e) => updateFields.mutate({ issueId: issue.id, fields: { Priority: e.target.value } })}
							className="h-6 text-xs rounded border bg-transparent px-1 w-32"
						>
				...
				<div className="px-3 py-3 border-b border-border flex flex-col gap-2">
					...
					<div className="flex gap-1.5">
						<DropdownMenu>
							<DropdownMenuTrigger asChild>
								<Button variant="outline" size="sm" className="flex-1 justify-between font-normal h-7 min-w-0 text-xs">
										...
										{selectedProject ? (
											<>
												<ProjectThumbnail
													...
													className="size-3.5"
												/>
												<span className="truncate">{String(selectedProject.name)}</span>
											</>
										) : (
							...
							<DropdownMenuContent align="start" className="w-[--radix-dropdown-menu-trigger-width]">
								{recentProjects.filter((p) => p.id).map((project) => (
									<DropdownMenuItem key={project.id} onClick={() => setSelectedProjectId(project.id)} className="flex items-center gap-2">
										<ProjectThumbnail
					...
					/>
					<div className="flex items-center justify-between">
						<Label htmlFor="sidebar-auto-run" className="text-xs font-normal">Auto-run</Label>
						<Switch id="sidebar-auto-run" checked={autoRun} onCheckedChange={setAutoRun} />
					</div>
					<button
						type="button"
						className="text-xs text-muted-foreground hover:text-foreground transition-colors mt-1"
						disabled={updateState.isPending}
						onClick={() => updateState.mutate({ issueId: issue.id, state: isClosed ? "Open" : "Closed" })}
					>
						{updateState.isPending ? "..." : isClosed ? "Reopen Issue" : "Close Issue"}
					</button>
				</div>
				...
				<div className="px-3 py-3">
					...
					<div className="flex items-start gap-2 mb-3">
						<div className="size-5 rounded-full bg-muted flex items-center justify-center text-[10px] font-medium shrink-0 mt-0.5">S</div>
					...
					{(comments ?? []).map((comment: { id: number; content: string; date: string }) => (
						<div key={comment.id} className="flex items-start gap-2 mb-3">
							<div className="size-5 rounded-full bg-muted flex items-center justify-center text-[10px] font-medium shrink-0 mt-0.5">C</div>
							<div className="min-w-0">
								<span className="text-xs text-muted-foreground">{new Date(comment.date).toLocaleDateString("de-DE")}</span>
								<p className="text-xs mt-0.5 whitespace-pre-wrap break-words">{String(comment.content)}</p>
							</div>
						</div>
					))}
				</div>
				...
				<textarea
					...
					onKeyDown={(e) => {
						if (e.key === "Enter" && (e.metaKey || e.ctrlKey) && commentDraft.trim()) {
							addComment.mutate({ issueId: issue.id, content: commentDraft.trim() });
						}
					}}
				/>
				<Button
					...
					onClick={() => addComment.mutate({ issueId: issue.id, content: commentDraft.trim() })}
				>
