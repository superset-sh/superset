...
export const Route = createFileRoute(
	"/_authenticated/_dashboard/v2-workspace/$workspaceId/old/",
)({
	component: V2WorkspaceOldPage,
});

function V2WorkspaceOldPage() {
...
function V2WorkspaceOldContent({
	canOpenInApp,
	projectId,
	projectName,
	workspaceBranch,
	...
	const { data: defaultApp } = electronTrpc.projects.getDefaultApp.useQuery(
		{ projectId },
		{ enabled: Boolean(projectId), staleTime: 30000 },
	);
	...
	const commandPalette = useCommandPalette({
		...
		onSelectFile: ({ close, filePath, targetWorkspaceId }) => {
			...
			if (targetWorkspaceId !== workspaceId) {
				void navigate({
					to: "/v2-workspace/$workspaceId/old",
				...
				<div className="border-b border-border px-6 py-4">
					<div className="mb-3 flex items-start justify-between gap-4">
						...
						<Button asChild size="sm" variant="outline">
							<Link params={{ workspaceId }} to="/v2-workspace/$workspaceId">
								Pane Viewer
							</Link>
						</Button>
					</div>
