...
export function useRemoteHostStatus(
	...
	const localMachineReady = machineId != null;
	const isLocal =
		workspace != null && machineId != null && workspace.hostId === machineId;
	const shouldCheckRemote = workspace != null && localMachineReady && !isLocal;
	const filterMachineId = shouldCheckRemote ? hostId : "";
	...
	return getRemoteHostStatus({
		...
		localMachineReady,
		isLocal,
		liveQueryReady: isReady,
		hostName: hostRow?.name,
		hostIsOnline: hostRow?.isOnline,
		hostInfoStatus: infoQuery.status,
		hostVersion: infoQuery.data?.version,
	});
}
