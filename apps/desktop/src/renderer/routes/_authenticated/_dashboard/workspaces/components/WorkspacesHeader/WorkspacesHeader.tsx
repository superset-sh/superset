...
import { BoardColumnIcon } from "renderer/routes/_authenticated/_dashboard/workspaces/components/BoardColumnIcon";
import type {
	WorkspaceCreatorOption,
	WorkspaceHostFilterOption,
	WorkspaceProjectOption,
} from "renderer/routes/_authenticated/_dashboard/workspaces/hooks/useAccessibleWorkspaces";
import {
...
} from "renderer/routes/_authenticated/_dashboard/workspaces/stores/workspacesFilterStore";
import { BOARD_COLUMN_LABELS } from "renderer/routes/_authenticated/_dashboard/workspaces/utils/deriveBoardColumn";
import { PRIcon } from "renderer/screens/main/components/PRIcon/PRIcon";
import { WorkspaceProjectIcon } from "../WorkspaceProjectIcon";
import { DeviceOptionLabel } from "./components/DeviceOptionLabel";

const PR_STATE_LABELS: Record<WorkspacesPrStateFilter, MessageDescriptor> = {
...
interface WorkspacesHeaderProps {
	hostOptions: WorkspaceHostFilterOption[];
	projectOptions: WorkspaceProjectOption[];
	creatorOptions: WorkspaceCreatorOption[];
	hostsById: Map<
		string,
											...
											onCheckedChange={() =>
												setAgentStatusFilters(
													toggleIn(
														agentStatusFilters,
														status,
													) as WorkspacesAgentStatusFilter[],
												)
											}
										>
											{i18n._(V2_WORKSPACES_AGENT_STATUS_LABELS[status])}
							...
							<DropdownMenuSub>
								...
								<DropdownMenuSubContent className="min-w-[10rem]">
									<DropdownMenuRadioGroup
										value={pinFilter}
										onValueChange={(next) =>
											setPinFilter(next as WorkspacesPinFilter)
										}
									>
					...
					<DropdownMenu modal={false}>
						...
						<DropdownMenuContent align="end" className="min-w-[12rem]">
							...
							<DropdownMenuRadioGroup
								value={sortMode}
								onValueChange={(next) =>
									setSortMode(next as WorkspacesSortMode)
								}
							>
							...
							<DropdownMenuRadioGroup
								...
								onValueChange={(next) =>
									setArchivedWindow(next as WorkspacesArchivedWindow)
								}
							>
								{(
									Object.keys(
										ARCHIVED_WINDOW_LABELS,
									) as WorkspacesArchivedWindow[]
								).map((window) => (
