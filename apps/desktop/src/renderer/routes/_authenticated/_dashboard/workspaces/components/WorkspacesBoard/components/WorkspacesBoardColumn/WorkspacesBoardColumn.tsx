...
import { WorkspacesBoardCard } from "../WorkspacesBoardCard";

interface WorkspacesBoardColumnProps {
	column: BoardColumnKey;
	workspaces: AccessibleV2Workspace[];
}

export function WorkspacesBoardColumn({
	column,
	workspaces,
}: WorkspacesBoardColumnProps) {
	return (
		...
		<div className="flex w-[280px] min-w-[280px] shrink-0 flex-col rounded-lg bg-muted/30">
			...
			<div className="flex min-h-[60px] flex-1 flex-col gap-2 overflow-y-auto px-2 pb-2">
				{workspaces.map((workspace) => (
					<WorkspacesBoardCard key={workspace.id} workspace={workspace} />
				))}
			</div>
		</div>
	);
