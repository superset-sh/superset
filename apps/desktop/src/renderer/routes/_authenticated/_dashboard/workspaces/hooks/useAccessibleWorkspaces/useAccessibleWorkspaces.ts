...
export type WorkspaceHostType = "local-device" | "remote-device";

/** Host vocabulary — includes merge-queue, unlike the old cloud join. */
export type WorkspacePrState =
	| "open"
	| "merged"
	| "closed"
	| "draft"
...
export type WorkspacePrReviewDecision =
	| "approved"
	| "changes_requested"
	| "pending";

export type WorkspacePrChecksStatus =
	| "none"
	| "pending"
	| "success"
	| "failure";

export interface WorkspacePrSummary {
	prNumber: number;
	title: string;
	url: string;
	state: WorkspacePrState;
	checksStatus: WorkspacePrChecksStatus;
...
export interface WorkspaceHostFilterOption {
	hostId: string;
	hostName: string;
	isOnline: boolean;
	isLocal: boolean;
...
export interface WorkspaceCreatorOption {
	userId: string;
	name: string;
	image: string | null;
...
interface UseAccessibleV2WorkspacesResult {
...
export function useAccessibleWorkspaces(
	...
	const { data: sidebarStateRows = [] } = useLiveQuery(
		(q) =>
			q
				.from({ sidebarState: collections.workspaceLocalState })
				.select(({ sidebarState }) => ({
	...
	const { data: sidebarProjectRows = [] } = useLiveQuery(
		(q) =>
			q
				.from({ sidebarProject: collections.sidebarProjects })
	...
	const prSummaryEntries = useMemo<[string, WorkspacePrSummary][]>(() => {
		const entries: [string, WorkspacePrSummary][] = [];
	...
	const diffStatsEntries = useMemo<[string, WorkspaceDiffStats][]>(() => {
		const entries: [string, WorkspaceDiffStats][] = [];
	...
	const enriched = useMemo<AccessibleV2Workspace[]>(() => {
		...
		for (const row of rows) {
			...
			const hostType: WorkspaceHostType =
	...
	const hostOptions = useMemo<WorkspaceHostFilterOption[]>(() => {
	...
	const projectOptions = useMemo<WorkspaceProjectOption[]>(() => {
		const byProject = new Map<string, WorkspaceProjectOption>();
	...
	const creatorOptions = useMemo<WorkspaceCreatorOption[]>(() => {
		const byCreator = new Map<string, WorkspaceCreatorOption>();
