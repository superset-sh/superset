...
import { useProjectLocalMetaStore } from "renderer/stores/project-local-meta";
...
export function WorkspacesList({ workspaces, isReady }: WorkspacesListProps) {
	...
	const resetFilters = useWorkspacesFilterStore((state) => state.reset);
...
function StatusSectionGroup({
	...
	const persistedCollapsed = useProjectLocalMetaStore(
		(state) => state.projects[collapseKey]?.isCollapsed ?? false,
	);
	const toggleCollapsed = useProjectLocalMetaStore(
