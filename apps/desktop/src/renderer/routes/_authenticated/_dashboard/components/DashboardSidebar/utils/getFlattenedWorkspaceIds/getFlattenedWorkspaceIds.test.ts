...
import { getFlattenedWorkspaceIds } from "./getFlattenedWorkspaceIds";
...
type Collections = Pick<
	AppCollections,
	"sidebarProjects" | "sidebarSections" | "workspaceLocalState"
>;
...
function makeCollections(args: {
	...
	return {
		sidebarProjects: fakeCollection(
			[
				{
		...
		sidebarSections: fakeCollection(
			args.sections ?? [],
			(row) => row.sectionId,
		),
		workspaceLocalState: fakeCollection(
...
describe("getFlattenedWorkspaceIds with tag folders", () => {
