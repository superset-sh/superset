...
import { COLLECTION_DROP_ROOT } from "../../utils/collectionDnd";
 ...
 * dragged back out of a collection. While a project drag is active it grows to
 * claim all empty space below the collections — otherwise "drop in the empty area"
 * would resolve to the nearest collection header instead of the root.
 *
...
export function RootDropZone({ children }: RootDropZoneProps) {
	...
	const { setNodeRef, isOver } = useDroppable({ id: COLLECTION_DROP_ROOT });
	return (
		<div
			ref={setNodeRef}
			className={cn(
