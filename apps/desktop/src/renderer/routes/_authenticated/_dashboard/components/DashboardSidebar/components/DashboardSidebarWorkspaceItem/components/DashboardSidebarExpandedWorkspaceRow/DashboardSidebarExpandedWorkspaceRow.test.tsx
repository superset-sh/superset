...
describe("DashboardSidebarExpandedWorkspaceRow archive action", () => {
	...
	test("an archived worktree row offers Restore and Delete, without opening", () => {
		const onRestore = mock(() => {});
		const onClose = mock(() => {});
		const { page, onClick, onKeyboardActivate } = renderRow({
			workspace: buildWorkspace({ archivedAt: Date.now() }),
			onClick: undefined,
			onKeyboardActivate: undefined,
		...
		// Both are visible without a hover: they are all the row affords.
		const restore = page.getByLabelText("Restore workspace");
		expect(restore.closest(".hidden")).toBeNull();
		fireEvent.click(restore);
		expect(onRestore).toHaveBeenCalledTimes(1);
		fireEvent.click(page.getByLabelText("Delete workspace"));
		expect(onClose).toHaveBeenCalledTimes(1);
		fireEvent.click(page.getByText("feature-row"));
