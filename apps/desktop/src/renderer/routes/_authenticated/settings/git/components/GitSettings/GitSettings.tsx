...
import { useDefaultWorktreePath } from "../../../components/WorktreeLocationPicker/useDefaultWorktreePath";
...
/**
 * Git settings — host-wide branch-prefix default for whichever device the
 * picker has selected. Per-host setting; the dropdown only appears when the
 * user has 2+ devices in this org.
 */
export function GitSettings({ hostId }: GitSettingsProps) {
	const { t } = useLingui();
	...
	const worktreeQuery = useWorktreeLocationSettings(targetHostUrl, {
		enabled: isHostOnline,
	});
	const setWorktreeBaseDir = useSetWorktreeBaseDir(targetHostUrl);
