 ...
 *
 * Nearly all projects now render through the settings page
 * (`ProjectSettings`). A fully-built icon picker (`IconUploadField`) exists
 * in the tree with backing tRPC mutations, but it was never wired into
 * `ProjectSettings` — so there is no longer any UI to set a project icon.
 *
 * This test renders `ProjectSettings` with its data/host dependencies
...
mock.module("./components/ScriptsEditor", () => ({
	ScriptsEditor: () => null,
}));
...
// assertion checks that ProjectSettings wires the section in, not how the
...
// ProjectSettings wires the section in and seeds it from the host row.
...
// checks the wiring (that ProjectSettings renders it) rather than the
// picker's internals.
...
describe("ProjectSettings", () => {
	test("renders the project icon picker so users can set an icon (#5843)", () => {
		const markup = renderToStaticMarkup(
			<ProjectSettings projectId="project-1" hostId="host-1" />,
		);
