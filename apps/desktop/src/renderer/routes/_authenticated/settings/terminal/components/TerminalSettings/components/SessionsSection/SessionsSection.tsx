// Settings → Terminal → Manage daemon section.
//
...
// per-row kill. Restart already achieves the kill-all effect;
...
export function SessionsSection() {
	...
	return (
		...
		>
			<SessionsSectionInner />
		</WorkspaceClientProvider>
	);
}

function SessionsSectionInner() {
	const { t } = useLingui();
	const searchQuery = useSettingsSearchQuery();
	...
	if (updateStatusQuery.error) {
		console.error(
			"[SessionsSection] getUpdateStatus error:",
			updateStatusQuery.error,
		);
	}
	if (sessionsQuery.error) {
		console.error("[SessionsSection] listSessions error:", sessionsQuery.error);
	}
