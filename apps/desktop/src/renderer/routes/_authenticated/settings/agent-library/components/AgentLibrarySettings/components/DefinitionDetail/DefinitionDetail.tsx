...
export function DefinitionDetail({
	...
	// Initialize/reset the draft whenever a fresh detail arrives.
	useEffect(() => {
		if (!detail) return;
			...
			<div className="flex items-start justify-between gap-3">
				<div className="min-w-0">
					<div className="flex items-center gap-2">
						<h2 className="text-xl font-semibold truncate">{summary.name}</h2>
				...
				<div className="flex items-center gap-2 shrink-0">
					<TransferMenu summary={summary} scopes={scopes} onDone={onMutated} />
					<Button
						variant="ghost"
						size="sm"
			...
			{hasConflict && (
				<div className="rounded-md border border-destructive/50 bg-destructive/10 p-3 text-sm space-y-2">
					<p className="select-text cursor-text">
						This file changed on disk since it was loaded. Reload to pick up the
						new content (your unsaved edits here will be discarded), or keep
						editing and resolve manually via "Edit raw".
					</p>
					<Button
						variant="outline"
						size="sm"
