...
2. The terminal persistence setting (`settings.persistTerminal`) is treated as "requires restart" today; we keep that behavior for this refactor.
...
9. **Registry invalidation / settings:** terminal persistence remains "requires restart" (existing behavior). The runtime registry is process-scoped and does not reconfigure live if `settings.persistTerminal` is toggled while the app is running.
