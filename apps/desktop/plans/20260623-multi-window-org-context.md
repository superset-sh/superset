...
- [x] (2026-06-23 16:00Z) Milestone 2: per-window org context. Added `lib/trpc/context.ts` (`createTrpcContext` → `ctx.senderWindow` via `BrowserWindow.fromWebContents`), typed the root tRPC context, wired `createContext` into the IPC handler. Added `window.getActiveOrg`/`setActiveOrg`/`openNew` and made OS-control procedures act on `ctx.senderWindow`. Renderer: `collections.ts` sends `x-superset-organization-id` from a per-window `currentOrgId`; `CollectionsProvider` resolves org from the window registry (session fallback for first window), switch is window-local (dropped `authClient.organization.setActive`), and exposes `activeOrganizationId`; `OrganizationDropdown` checkmark now reflects the window's org. Skipped `layout.tsx` — its redirect gate only needs "user has any org", which the untouched session default still answers. Typecheck + lint clean; registry tests green. Commit e2eaeef63.
- [x] (2026-06-23 18:05Z) Bugfix during verification: per-window org was re-resolved on every `getActiveOrg` read; a transient empty read fell back to the session org AND persisted it, snapping a switched window back. Fixed with init-once resolution (commit 0e7bb90e0).
- [x] (2026-06-23 20:55Z) Root-cause of "workspaces identical across orgs": the user is on the **v1 UI**, whose workspace list is machine-local (org-agnostic). Per-window org is a **v2** feature (Decision D7). Verified the actual behavior on v2 (below).
- [x] (2026-06-23 22:39Z) Milestone 3: per-window title. New `WindowTitle` component sets `document.title = "<Org> — Superset"` (mirrored to native window title), mounted in `CollectionsProvider`. Commit d6b177913.
- [x] (2026-06-23 22:55Z) Milestone 4: restore-all-windows. `window-state.ts` adds `loadWindows`/`saveWindows`/`isValidPersistedWindow` + legacy→multi migration; `main.ts` `persistOpenWindows()` (on move/resize, single-window close, and before-quit snapshot) + `restoreWindows()` wired into `makeAppSetup`. +7 unit tests. Commit aec607d7b.
- [x] (2026-06-23 22:55Z) Runtime verification via CDP (dev on 4000 range, `RENDERER_REMOTE_DEBUG_PORT=9222`, opted into v2): (a) New Window opens a 2nd window; (b) switching window 1 → org B left window 0 on org A (per-window isolation, `senderWindow` routes correctly); (c) titles distinct ("Ash 2 — Superset" / "Local Admin's Team — Superset"); (d) `persistOpenWindows` wrote the real open-window set; (e) planting a 2-window `windows-state.json` + relaunch restored both windows on their orgs.
- [ ] Pre-PR cleanup: remove the local `.env`/port changes are local-only (gitignored); comment on issue #4018 with the v2 slice; open PR from `ashbrener/superset`. Validation pass (`bun run typecheck`/`lint`/`test`) before push.
- [ ] Validation pass: `bun run typecheck`, `bun run lint`, `bun test` all green; manual two-window acceptance.


## Surprises & Discoveries

- Observation (MAJOR — corrects the feasibility research): the desktop has **two UIs** — v1 (`src/renderer/screens/main/`) and v2 (`src/renderer/routes/_authenticated/_dashboard/`), gated by `useIsV2CloudEnabled()` (account-age cutoff `useIsV2OnlyUser`, or the Settings → Experimental `optInV2` override). **Per-window org only changes what the user sees on v2.**
  Evidence: v1's workspace list (`screens/main/components/WorkspacesListView`) is fed by `electronTrpc.workspaces.getAllGrouped` — a main-process query over the **local SQLite DB** (`src/lib/trpc/routers/workspaces/procedures/query.ts:162`) with **no org filter**. The local-db `projects`/`worktrees`/`workspaces` tables (`packages/local-db/src/schema/schema.ts`) have **no `organizationId` column**; org exists only as a single global `settings.activeOrganizationId` plus cloud tables. Nothing re-syncs that list on org change. So v1 workspaces are machine-local and identical across orgs *by design* — switching org never blanked them, and the per-window change did not break that. The v2 sidebar (`DashboardSidebar` → `useDashboardSidebarData`) DOES read org-scoped Electric collections via the per-window `CollectionsProvider`, so per-window org is meaningful there. The original feasibility agent inspected the cloud/`CollectionsProvider` path and missed the v1 local-db path.
  Decision impact: this feature targets **v2** (see Decision D7). Verify on v2 (opt in via Settings → Experimental).
...
- Decision **D7 (2026-06-23)**: Per-window organization context targets the **v2 cloud UI**, not v1.
  Rationale: v1 workspaces are machine-local (local-db, no org column) and identical across orgs by design, so per-window org has no effect there; v2 workspaces are org-scoped Electric collections that the per-window `CollectionsProvider` already drives. v2 is also the product's direction (new accounts are v2-only). Org-scoping v1's local worktrees would need a local-db schema migration and fights v1's local-first design — out of scope and unlikely to be accepted upstream. Verification and the PR scope are v2-only. For v1 multi-platform, the separate-instances workaround remains the answer.
  Date/Author: 2026-06-23 / plan author (user steer: "what's recommended").
