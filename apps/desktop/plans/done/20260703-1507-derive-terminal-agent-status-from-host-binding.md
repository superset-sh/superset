...
- [x] (2026-07-03 17:40Z) CDP end-to-end verification against the live dev app (real hook POSTs to host-service, assertions at DB + DOM + screenshot layers). Verified: hook → binding transitions (Start/PermissionRequest/Stop) in host SQLite; working → amber dotted-spinner icon on the sidebar workspace row; permission → red badge dot; unseen Stop → green review dot; **review survives a full renderer reload** (the headline stale-dot fix); review clears via the workspace-row click (markWorkspaceTerminalsSeen); SessionEnd/Detached deletes the binding. All five pre-existing bindings on the dev host have active workspace-owned sessions (startup deleteDefunct ran clean).
- [ ] Remaining human acceptance: visible-Stop seen-marking, interrupt synthetic Stop, and the exit-leak UI reaction could NOT be driven over CDP — this dev environment has a pane-layout sync bug (panes vanish from the layout within seconds of creation, so no mounted TerminalPane and no pty input path). The exit-leak join semantics are unit-tested in `packages/host-service/src/terminal-agents/persistence.test.ts`; the other two need a quick `bun dev` pass with a real agent.
