...
export async function POST(request: Request) {
	...
	try {
		...
		// Upsert all repos in parallel (bounded to 10 concurrent DB writes).
		await pLimit(
			repos.map((repo) => () =>
				db
		...
		// Process repos sequentially (each paginates PRs) but parallelise per-PR
		// check-run fetches + DB writes within each repo to cut wall-clock time.
		for (const repo of repos) {
			...
			// Fetch check-runs and upsert each PR concurrently (5 at a time) to
			// avoid the O(N) sequential GitHub API calls that cause 300 s timeouts
			// on installations with many PRs.
			await pLimit(
				prs.map((pr) => async () => {
