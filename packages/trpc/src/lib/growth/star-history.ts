...
// Next augments RequestInit with its own data-cache options, and this module is
// read from a Next app (marketing) and from the API's server package alike, so
// the one option we set is declared here rather than borrowed from that
// augmentation. Outside Next the field is inert and the caller's own cache (the
// admin dashboard reads through the growth metric cache) does the work.
...
async function fetchRepoMeta(
	...
	try {
		...
		console.error("[star-history] Error fetching repo metadata:", error);
		return null;
	}
}
...
// `token` is a read-only PAT (no scopes needed). Callers own it, so the
// marketing app and the API can each read it from their own environment
