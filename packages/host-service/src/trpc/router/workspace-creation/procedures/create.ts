...
export const create = protectedProcedure
	...
	.mutation(async ({ ctx, input }) => {
		...
		// Fire-and-forget AI rename from any naming context (composer prompt
		// and/or linked issue/PR titles). A single structured-output call
		// generates both a display title and a kebab-case branch name, and
		// we apply each independently. Electric syncs updates to the
		// renderer via v2_workspaces, so the pending/workspace page updates
		// in place once the model responds.
		//
		// Name precedence (matches renderer `resolveNames`):
		//   1. user-typed title → skip AI rename (flag = false)
		//   2. friendly fallback + (prompt OR linked titles) → AI rename
		//   3. friendly fallback, no naming context → keep fallback
		//
		...
		const linkedIssueTitles = input.linkedContext?.linkedIssueTitles?.filter(
			(t) => t.trim().length > 0,
		);
		const linkedPrTitle = input.linkedContext?.linkedPrTitle?.trim();
		const allowAiRename = input.names.workspaceNameWasAutoGenerated !== false;
		const hasNamingContext =
			Boolean(composerPrompt) ||
			(linkedIssueTitles && linkedIssueTitles.length > 0) ||
			Boolean(linkedPrTitle);
		if (hasNamingContext && allowAiRename) {
			void applyAiWorkspaceRename({
				...
				linkedIssueTitles,
				linkedPrTitle: linkedPrTitle || undefined,
			}).catch((err) => {
				console.warn(
