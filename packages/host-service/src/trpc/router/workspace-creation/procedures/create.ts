...
import {
	clearSetupWorkspaceName,
	syncWorkspaceNameArtifacts,
...
export const create = protectedProcedure
	...
	.mutation(async ({ ctx, input }) => {
		...
		const shouldApplyAiRename = Boolean(composerPrompt && allowAiRename);
		if (input.composer.runSetupScript) {
			if (shouldApplyAiRename) {
				clearSetupWorkspaceName(worktreePath);
			} else {
				writeSetupWorkspaceName(worktreePath, input.names.workspaceName);
			}
		}
					...
					if (input.composer.runSetupScript) {
						return syncWorkspaceNameArtifacts({
							...
							fallbackWorkspaceName:
								rename?.workspaceName ?? input.names.workspaceName,
						});
					}
				})
				.catch((err) => {
					...
					if (input.composer.runSetupScript) {
						return syncWorkspaceNameArtifacts({
							...
							workspaceId: cloudRow.id,
							worktreePath,
							fallbackWorkspaceName: input.names.workspaceName,
						});
					}
				});
			if (input.composer.runSetupScript) {
				const refreshTimer = setTimeout(() => {
					void syncWorkspaceNameArtifacts({
						ctx,
						workspaceId: cloudRow.id,
						worktreePath,
						fallbackWorkspaceName: input.names.workspaceName,
		...
		if (input.composer.runSetupScript) {
			const { terminal, warning } = startSetupTerminalIfPresent({
				...
				workspaceNamePending: shouldApplyAiRename,
			});
			if (warning) {
