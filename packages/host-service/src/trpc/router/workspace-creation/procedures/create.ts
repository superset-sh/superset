...
export const create = protectedProcedure
	...
	.mutation(async ({ ctx, input }) => {
		// Single seam for clearing the progress entry: any throw inside
		// the body funnels through this finally so the renderer never
		// sees a stuck "active" step. The scattered clearProgress calls
		// inside are now redundant but harmless (the call is idempotent).
		try {
