...
describe("AcpSessionEventTranslator", () => {
	...
	test("setting catalogs map selects, flatten groups, synthesize mode, skip booleans", () => {
		...
		if (state.kind !== "state") throw new Error("expected state frame");
		const catalogs = settingOptionsFromScopedState(state.state);
	...
	test("a catalog-only change still appends a settings event", () => {
		const translator = makeTranslator();
		const env = makeJournal();
		const modelOption = (models: string[]) => ({
			id: "model",
			name: "Model",
			category: "model" as const,
			type: "select" as const,
		...
		const first = translateAll(translator, [
			env(stateFrame({ configOptions: [modelOption(["sonnet"])] })),
		]);
		expect(only(first, "settingsUpdated")).toHaveLength(1);
		// Same values, wider catalog: the composed session (the catalogs'
		// carrier) is only re-pushed when the log moves, so this must append.
		const widened = translateAll(translator, [
			env(stateFrame({ configOptions: [modelOption(["sonnet", "opus"])] })),
		]);
		expect(only(widened, "settingsUpdated")).toHaveLength(1);
		validateAndFold([...first, ...widened]);
	});
