...
describe("CanonicalSessionsRuntime", () => {
	...
	test("offline sessions: reads resurrect and answer the recent tail", async () => {
		const port = new FakeAcpPort();
		const runtime = makeRuntime(port);
		...
		// Cold read: resurrects (the same session/load the zero-head subscribe
		// would run moments later) and answers the replayed tail in ONE
		// snapshot — a device's first paint must be the full last page, never
		// an event-by-event replay over the socket.
		const cold = await runtime.getSession({ sessionId: "session-off" });
		sessionSnapshotSchema.parse(cold);
		expect(cold.session.runState).not.toBe("offline");
		expect(cold.head).not.toBe("c000000000000");
		expect(cold.hasOlderEvents).toBe(false);
		const coldTexts = cold.recentEvents
		...
		expect(coldTexts).toContain("from before the restart");
		expect(coldTexts).toContain("old reply");
