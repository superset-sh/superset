...
export const settingOptionChoiceSchema = z.object({
...
/**
 * A harness-reported settings catalog: the choices behind one settings field.
 * `kind` names the `SessionSettings` field the catalog backs (model/mode/
 * effort map onto `updateSession.settings`; `other` entries key into
 * `settings.configuration` and are display-only for now). Catalogs are
 * session-level metadata composed from live harness state — they ride the
 * session entity (snapshots + sessionUpsert), never the event log.
 */
export const settingOptionSchema = z.object({
	id: identifierSchema,
	name: z.string().min(1).max(1_024),
	kind: z.enum(["model", "mode", "effort", "other"]),
	currentValue: identifierSchema.nullable(),
	options: z.array(settingOptionChoiceSchema).max(100),
});
...
export const sessionSchema = z.object({
	...
	// default([]) so snapshots from hosts predating catalogs still parse.
	settingOptions: z.array(settingOptionSchema).max(100).default([]),
...
export type SettingOptionChoice = z.infer<typeof settingOptionChoiceSchema>;
