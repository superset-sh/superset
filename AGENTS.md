...
3. **Shared command source** - keep command definitions in `.agents/commands/` only. `.claude/commands`, `.cursor/commands`, and `.codex/commands` should be symlinks to `../.agents/commands`.
4. **Workspace MCP config** - keep shared workspace MCP servers in `.mcp.json`; `.cursor/mcp.json` and `.codex/mcp.json` should be links to `../.mcp.json` (single source of truth).
