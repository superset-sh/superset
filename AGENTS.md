...
4. **Workspace MCP config** - keep shared MCP servers in `.mcp.json`; `.cursor/mcp.json` should link to `../.mcp.json`. Codex uses `.codex/config.toml` (run with `CODEX_HOME=.codex codex ...`). OpenCode uses `opencode.json` and should mirror the same MCP set using OpenCode's `remote`/`local` schema. Kilo Code uses `.kilocode/mcp.json` (VS Code extension) and `kilo.json` (CLI). Both should mirror the same MCP set from `.mcp.json`. Kilo Code skills in `.kilocode/skills/` are converted from `.agents/commands/` (cannot use symlinks due to different directory structure).
